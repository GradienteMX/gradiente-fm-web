...
### Reconocimiento — lo que había

- **`/admin` veía UN tipo de contenido.** `getAllEventsAdmin()` está clavado a `.eq('type','evento')`: 450 de 601 filas. Los otros 151 items (mixes, reseñas, editoriales, opiniones, artículos, listas, noticias) **no tenían superficie admin de ningún tipo**.
- **CERO superficie HP/HL.** Ni tile, ni columna, ni palanca. La única forma de preguntar «¿la vida media de `mix` es demasiado larga?» era mirar el mosaico y adivinar — con 61 usuarios y ~17 interacciones/día, adivinar durante meses.
- **`hp_events` era una cola, no un registro.** `apply_hp_rollup()` plegaba el peso en `items.hp` y **BORRABA la fila**. La columna `kind` —el único sitio donde existió jamás el desglose clic/apertura/guardado/comentario— se descartaba en cada tick y no persistía en ninguna parte. **~2.110 eventos destruidos, irrecuperables, sin backfill posible.** El lado usuario sí retiene desde mayo (0020 hizo exactamente esto en `user_hp_events`): cuatro meses de asimetría que nadie había notado porque nada la leía.
- **Una puerta trasera sin auditar sobre `items.hp`.** `authenticated` tenía un `UPDATE` de tabla completa sobre `items` sin entradas a nivel de columna, y `items_staff_update` sólo filtra a nivel de FILA. Las **39 cuentas elevadas** podían hacer `supabase.from('items').update({ hp: 999 })` desde una sesión de navegador: sin ruta, sin auditoría, sin rastro.
- **`audit_log` llevaba cuatro meses sin un solo escritor.** El comentario de `0002_rls.sql` que dice que los inserts llegan por triggers **nunca fue cierto**.
- **Políticas RLS esperando un consumidor que no llegaba**: `hp_events_admin_read` y `user_hp_events_admin_read` existen desde 0002 y no las usaba nadie.
- **`hp_events` no tenía índice sobre `item_id`.** Sólo `pkey(id)` y `window_idx(created_at)`. El propio `join` del rollup era barato SÓLO porque la tabla estaba casi vacía entre ticks.
- **La página y la barra de pestañas se contradecían**: ante un `?tab=` desconocido la página caía a `invites` mientras la barra casteaba el parámetro crudo a su unión, así que `?tab=bogus` pintaba contenido de INVITACIONES **sin ninguna pestaña pestillada**.
- **Seis copias de `FOCUS_RING`** y el bloque de gate admin copiado en las siete rutas `/api/admin` (cinco inline, dos como helper local con dos nombres distintos). `KIND_WEIGHTS` vivía como const privada dentro de `app/api/hp-events/route.ts`, ilegible desde cualquier UI.
- **Moderación sin sustrato.** Prod tiene 31 comentarios, 4 tombstoned, y **tres de esos cuatro son autoborrados del propio autor**: exactamente UNA acción de moderador en cuatro meses. Nada en el producto era reportable — no había tabla, ni gesto, ni cola. Un tile «REPORTES · N» cableado al conteo de tombstones habría **cuadruplicado** la moderación real.
- **Números falsos en comentarios**: la consulta de franjas afirmaba «<20 incluso a escala» (prod tiene 78), y `invite_codes` traía `.limit(50)` sobre 217 códigos, así que un código canjeado viejo caía fuera de la lista y se leía como «nunca invitado».

### Las cuatro decisiones

**1 · Exención de instrumento — [[Admin Instrument Exemption]].** `/admin` puede mostrar HL en crudo. Nada más puede. La exención se escribe ESTRECHA a propósito: superficie con gate de servidor a `role='admin'` (ocho cuentas), rutas `/api/admin/*` con `requireAdmin()` sobre la sesión del propio llamante, y **las leyes públicas intactas en todos los sitios para los que se escribieron** — `/u/[username]` sigue sin un solo número, el spine sigue mostrando el escalar sólo en su bloque PRIVADO, las cards siguen codificando prominencia como tamaño y posición. La razón es una sola: **no se puede afinar lo que no se puede ver**, y las constantes (vidas medias, `TYPE_SCORE_MULTIPLIER`, la banda de novedad, los umbrales de tier) se calibraron contra perfiles simulados y nunca contra producción. Lo que sigue PROHIBIDO aun aquí queda escrito: **cero atribución por usuario de las interacciones de item** — `hp_events` no tiene `user_id` POR DISEÑO y no debe ganarlo nunca, así que el desglose puede decir «14 aperturas» y jamás de quién; y el HL de usuario sigue privado e ininyectable.

**2 · `hp_events` deja de ser cola y pasa a libro mayor — [[HL Ledger]].** La historia empieza el día que se aplique la migración y **ni un segundo antes**. `LEDGER_EPOCH` es constante en `lib/hp/kinds.ts` y **toda ventana se recorta contra ella y lo dice** con una `MarginNote`: un tramo vacío de gráfica a la izquierda de esa fecha se leería como «nadie interactuó», que es justo lo contrario de la verdad. Regla dura del panel: **los CONTEOS salen de `base_weight`, las SUMAS de HL salen de `weight`**, porque `weight` ya viene multiplicado por el factor de novedad `m ∈ [0.6, 1.5]` (0025) y `weight/nominal` **no es un conteo** — se equivoca hasta ±50%. Donde el conteo no se puede saber, la celda imprime raya (`—`), nunca 0 y nunca una estimación con pinta de lectura.

**3 · La palanca HL existe, es auditada, y se detiene en los items.** `admin_adjust_item_hp()` fabrica bajo demanda estados del mosaico que el tráfico orgánico no producirá en meses. **No es un pulgar de curación**: para «que esto importe más» están `editorial`, `elevated` y `pinned`, que son declarativos y no fingen que hubo engagement. Cada llamada escribe fila de `audit_log` (actor · item · antes · después · aplicado · razón) **y** fila de `hp_events` de kind `admin_adjust` ya sellada `processed_at`, para que el HL inyectado salga como su propia banda y ninguna gráfica del producto pueda confundir inyección con alcance. **Nunca toca `users.engagement_hp`**: el HL de usuario es ganado y abre trofeos; inyectarlo fabricaría estatus no merecido. Y —la mitad que hace honesta a la otra— **§6 cierra la puerta trasera en la misma migración**: una palanca auditada al lado de una puerta sin auditar es decorativa.

**4 · `reports` es el sustrato de la cola, y un reporte es una señal, no un voto.** Índice único `(reporter_id, target_type, target_id)`: sin él un solo usuario inunda la cola. **Sin política DELETE en ninguna parte, a propósito**: un reporte resuelto o descartado es el registro de que alguien miró. `resuelto` («se actuó») y `descartado` («se revisó y no hacía falta») se conservan separados — colapsarlos pierde la diferencia entre «lo arreglamos» y «no estuvimos de acuerdo», que es exactamente lo que una cola existe para registrar. **Nada de consecuencia automática**: ni notificación, ni auto-ocultado, ni contador de strikes. Construir la mitad sancionadora de un sistema cuya mitad reportadora nunca se ha usado, con 61 usuarios, sería cromo.

### Migración `0049_admin_central.sql` — siete secciones

**Los cuerpos de §2/§3 se ramifican del `pg_get_functiondef` EN VIVO, no de los archivos en disco.** Los archivos mienten: el cuerpo de 0008 es V1 rancio y `apply_hp_rollup` se ha recreado cuatro veces (0008 → 0022 → 0024 → 0048). Ramificar de un archivo anterior reinstala en silencio `when 'partner' then 8760.0`, que desde el renombre ya no castea a `content_type`, y el cron **falla dentro del log de cron sin síntoma visible**. Es la misma trampa que documentó la entrada del 2026-08-25.

- **§1 · cola → libro mayor.** `processed_at` + `base_weight`. Dos índices que **tienen que aterrizar en ESTA migración, nunca después**: `hp_events_unprocessed_idx (created_at) where processed_at is null` (el camino caliente del rollup, parcial para que mida un tick por largo que crezca el registro) y `hp_events_item_kind_idx (item_id, kind, created_at desc)` (todo desglose). En cuanto la tabla es append-only, sin ellos tanto el join del rollup como cada consulta del panel hacen seq-scan sobre una tabla que crece.
- **§2 · `record_hp_event()` guarda también el peso base.** Byte-idéntico al cuerpo vivo salvo los dos INSERT. El multiplicador de novedad sigue sin devolverse al cliente y sin almacenarse: sólo los dos extremos de la multiplicación, que es suficiente para conteos honestos y nada más.
- **§3 · `apply_hp_rollup()` marca en vez de borrar — ⚠ EL CAMBIO DE MÁS RIESGO DE TODA LA MIGRACIÓN.** Son **dos ediciones que forman PAR**: (a) el cursor de apertura gana `where processed_at is null`, (b) el `delete from hp_events` final pasa a `update … set processed_at = now()`. El cuerpo vivo abre con un `select array_agg(id) into rolled_ids from hp_events;` **completamente sin filtrar**, cosa que era inofensiva SÓLO porque la última sentencia borraba todo lo que había leído. Con filas retenidas, ese mismo select **relee el registro ENTERO cada cinco minutos y lo vuelve a plegar** en `items.hp`: el HP compone sin límite, el orden del feed se corrompe en horas, y **NADA FALLA** — sin excepción, sin línea de log, sin ejecución de cron marcada como error. Enviar (b) sin (a) corrompe el feed en silencio. Toda otra línea del cuerpo es byte-idéntica a la 0048 viva.
- **§3 bis · se registra el decaimiento.** El decaimiento se calcula y nunca se guarda, y los anclajes contra los que se calculó los pisa el propio `UPDATE` de la línea de arriba: **no es reconstruible a posteriori**. Registrarlo hacia adelante es la única forma honesta de dibujar una línea neta. **Límite de alcance que el panel repite y que NUNCA debe etiquetarse como decaimiento del corpus**: cubre sólo los items que recibieron un evento en ese mismo tick, porque son las únicas filas que el rollup re-ancla. Ganancias + ese decaimiento = el cambio neto real de `items.hp`, que es lo que significa la línea neta; los otros ~590 items decaen invisibles. `weight` es NEGATIVO, `base_weight` queda NULL (una fila de decaimiento no tiene conteo de eventos), `processed_at` se sella al insertar.
- **§4 · barrido a 180 días.** `sweep_old_hp_events()` en pg_cron diario (`20 4 * * *`). Sólo filas PROCESADAS: una fila pendiente es HL sin aplicar y borrarla tiraría engagement en silencio. 180 días es seis veces la ventana más larga que ofrece la UI y es el techo al que recortan `clampDays()` y la ruta de stats — pedir más sólo puede añadir espacio vacío e insinuar que se perdió dato en vez de que nunca se guardó.
- **§5 · `admin_adjust_item_hp()`.** Decae a AHORA antes de ajustar (sumar sobre la columna cruda sumaría sobre un número rancio y el readout no cuadraría con el feed), suelo en 0, re-ancla `hp_last_updated_at` (una inyección escrita contra un anclaje de tres semanas se evaporaría en parte antes de que nadie la viera), `for update` sobre la fila (dos admins calibrando el mismo item leerían el mismo `before` y el segundo borraría al primero), `MAX_DELTA = 1000` como guarda antidedazo —el HP más alto que ha alcanzado un item en prod es ~71—, y **`revoke execute … from public`**: Postgres concede EXECUTE a PUBLIC por defecto, 0033 revocó de `anon`/`authenticated` mientras PUBLIC seguía teniéndolo y fue un no-op completo; 0034 tuvo que rehacerlo. Sin esas dos líneas la función es llamable desde una petición anónima.
- **§6 · se cierra el camino crudo a `items.hp`.** Un REVOKE a nivel de columna **no puede estrechar un grant de tabla** — Postgres mira primero el grant de tabla y el revoke de columna es un no-op silencioso. La única forma correcta: revocar `UPDATE` entero y re-concederlo columna por columna excepto las cinco de HP, con la lista calculada desde `information_schema` en tiempo de aplicación para que no pueda desviarse de la tabla real. **TRAMPA DE MANTENIMIENTO QUE ESTO CREA**: un futuro `alter table items add column x` cae FUERA del grant y `authenticated` no podrá actualizar `x` — con un error de permisos que **se lee como un problema de RLS**. El arreglo queda como función invocable, `select regrant_items_update();`, para que una migración futura no tenga que reconstruir el razonamiento. `service_role` y los SECURITY DEFINER (rollup, `harvest_item`, la palanca) corren como owner: COSECHAR y el cron siguen igual.
- **§7 · `reports`.** `reason`/`status` son TEXT con CHECK, **deliberadamente no enums**: la 0048 existe porque renombrar un valor de enum reventó cinco cuerpos plpgsql que guardaban el literal como texto; un CHECK se ensancha con un ALTER. `target_id` **sin FK**: es polimórfico sobre cinco tablas cuyas claves ni siquiera son del mismo tipo (`items.id` es text, `comments.id` es uuid), así que un reporte **sobrevive a su objeto** — borras el comentario y la fila queda apuntando a nada. Es la forma buscada, y la cola pinta esas filas como «OBJETO ELIMINADO» en vez de tirarlas. Las políticas envuelven `(select auth.uid())` para que el planner lo suba a initplan: `auth.uid()` pelado reintroduce exactamente la regresión que la 0035 arregló.

### El panel

- **Siete pestañas**: RESUMEN · CONTENIDO · EVENTOS · FRANJAS · USUARIOS · ACCESO · MODERACIÓN. Tres nuevas (RESUMEN, CONTENIDO, MODERACIÓN), una fusión (ACCESO absorbe INVITACIONES + ESPERA), tres que pasan igual.
- **Los `?tab=` viejos se ALIASAN, nunca se tiran** (`lib/admin/tabs.ts`): `invites`→ACCESO+sub · `espera`→ACCESO+sub · `users`→USUARIOS · `events`→EVENTOS · `franjas`→FRANJAS (sin cambio, pero listado porque `app/dashboard/page.tsx` enlaza ahí por nombre desde el retiro de la cola de aprobaciones). Desconocido → RESUMEN, que es el sitio honesto para «no sé a dónde querías ir». **`resolveAdminTab()` es el ÚNICO resolvedor y lo llaman los dos lados**, que es lo que mata la contradicción página-vs-barra.
- **RESUMEN es el instrumento**: cinco KPIs, FLUJO DE VIDA (HL ganada · HP creadores · decaimiento · neto), desglose por tipo de interacción, ATENCIÓN (cuatro filas de trabajo esperando, cada una con conteo real y enlace que va a algún sitio; conteo cero = sin enlace, no afordancia muerta) y salud del rollup (pendientes + último procesado; `stale` a los 15 min = dos ticks perdidos). **La ración de ácido se gasta aquí**: UN `AcidBlock` al pie con cuatro acciones de creación pares, no cuatro botones ácidos.
- **CONTENIDO es la pestaña que no existía**: los 601 items, no los 450 eventos. Fila = HL decaído, banda, banderas de estado, HL ganado, HL perdido por decaimiento, inyección admin si la hubo, y chispograma de 7 días. **El ranking se hace en Node y eso es OBLIGATORIO, no cómodo**: ordenar por `items.hp` en SQL enfrenta instantáneas de edades distintas, así que una fila rancia gana a una más fresca que de verdad está más caliente. Sólo `currentHp()` ordena esto y vive en TypeScript. La fila expande a un expediente que se pide al abrir (`GET /api/admin/items/[id]/stats`), no con la lista: 50 filas × un escaneo de registro + cuatro conteos = 250 consultas para la única fila que el operador abre.
- **Kit compartido** en `components/admin/kit.tsx`: reexporta los primitivos de espacios y añade `StatBlock`, `StatStrip`, `BarMeter`, `KindBreakdown`, `Sparkline`, `LineChart`, `ExpandableRow`, `LatchBar`, `useSingleOpen`. `LatchBar` deja **una** implementación de la gramática de pestillo donde había dos. `lib/hp/kinds.ts` levanta el vocabulario de kinds a un módulo legible desde UI — **con la regla en la cabecera de que ahí NO van clases de Tailwind**, porque los globs de `content` no escanean `lib/` y cualquier clase aparcada ahí se purga del build y falla en silencio.
- **`lib/api/requireAdmin.ts`**: una copia del gate en vez de siete. Usa SIEMPRE el cliente de sesión del llamante, nunca `createAdminClient()`; el service-role se alcanza aparte y sólo donde RLS bloquea de verdad una lectura admin legítima (conteos de guardados bajo `user_saves_self_only`, borradores ajenos) — nunca para saltarse el gate.

### El circuito de reportes

`POST /api/reports` (el lector presenta; la autorización real es `reports_insert_self`, así que un `reporter_id` falsificado lo rechaza la base aunque se puentee el handler) → tabla `reports` → `GET /api/admin/reports?estado=` → `PATCH /api/admin/reports/[id]`. **Ambas rutas admin van con `requireMod()`, NO con `requireAdmin()`**: `reports_read_staff` admite `private.auth_is_mod_or_admin()`, y una ruta más estricta que su propio RLS le daría 403 a un mod sobre filas que la base le habría enseñado. `canModerate()` en `lib/permissions.ts` es el espejo TS de ese predicado y se mueven juntos. El conteo de reportes abiertos viaja en el pestillo de MODERACIÓN **en todas las pestañas**, y degrada a `undefined` —sin conteo, sin punto— cuando la 0049 no está aplicada; nunca a un cero de relleno.

**La mitad que falta, dicha en la nota y no escondida**: no existe todavía gesto de reporte del lado lector. Nada en el producto llama a `POST /api/reports`. Hasta que aterrice un botón donde un lector lo alcance, MODERACIÓN está correcta y vacía — vacía a propósito, no sembrada.

### Tres defectos de EVENTOS encontrados en el pase

Están en el camino de guardado que ya existía, no en código nuevo, y los tres los produce el mismo helper:

1. **Cada guardado PUBLICA a la fuerza.** `contentItemToRow()` tiene `published: opts?.published ?? true` y `POST /api/admin/events` no pasa `opts`. Corregir una errata sobre un borrador raspeado lo pone en vivo en silencio. Peor: `ContentItem` **no tiene campo `published`** y `rowToContentItem()` no lo mapea, así que el editor ni siquiera puede mostrar el estado que está pisando.
2. **Cada guardado estampa `seed: false`.** Mismo helper. Ese flag es la marca de procedencia por la que está acotado el `--revert` del importador de Noche Negra y la distinción semilla-vs-real en el resto del sitio.
3. **El listado no tiene tope y va al revés.** `getAllEventsAdmin()` trae TODA fila `type='evento'` sin límite ni ventana, ordenada por `date` ascendente: 450 filas en prod, las más viejas primero, todas enviadas al cliente como filas editables con selectores de entidad. Lo que el operador necesita está al fondo.

### Estado

**La migración 0049 NO está aplicada.** Nada de HL en el panel nuevo muestra dato hasta que lo esté, y el conteo de MODERACIÓN degrada a nada. Instrucciones exactas de aplicación y checklist de verificación en [[Next Session]]. Sin `tsc` ni `next build` en esta sesión a propósito: había constructores concurrentes escribiendo archivos hermanos y habría fallado por ausencias, no por errores.
...
- **Cara póster (reposo)**: `PosterFace` compartida por los 3 tiers — arte full-bleed, título (+ subtítulo en md/lg) sobre el scrim slab, `DateChip` para eventos, `SavedBadge`. Nada más. `aria-hidden` (cero focusables adentro) para que el lector de pantalla oiga UNA card.
- **Cara densa (hover/foco)**: los layouts fase-B intactos, `absolute inset-0` → revelado por el **barrido «ficha»** (`.card-face-dense` en globals): clip-path de abajo hacia arriba + subida de 10px — la ficha técnica pasando por el rodillo, direccional y mecánica, nunca dissolve. (Primera iteración fue corte seco; Iker lo vetó como brusco — el barrido conserva el carácter print siendo suave.) Timing asimétrico: entrada 220ms `cubic-bezier(0.16,1,0.3,1)` con **60ms de hover-intent delay** (barrer el cursor por el mosaico no estroboscopea las cards que cruza); salida 130ms sin delay. El wrapper lleva `overflow-hidden` (recorta la subida a media pasada). `prefers-reduced-motion` → corte instantáneo. `group-focus-within` cubre el foco del wrapper Y el tab a los links internos: todo chip sigue alcanzable por teclado, con la misma pasada. Se quitó el subrayado hover de los títulos densos (la cara entera ES el estado hover) y el `group` anidado de los articles.
- **Táctil**: sin hover, el tap abre el overlay (misma info). El póster es la lectura móvil completa.
- **`PollCardCanvas` sube al wrapper** (encima de ambas caras): el chip VOTAR visible en reposo, y la boleta abierta sobrevive a que el puntero salga de la card.
- **Trampa de egress**: ambas caras pintan el mismo arte — `cardArtSizes()` produce UN string `sizes` por card usado por ambos plates; `sizes` divergentes = dos descargas de la misma imagen × ~140 cards.
- **Kicker dash (ronda 2, mismo día)**: el tipo volvió al reposo como raya de kicker — regla corta color-categoría (28px/36px-lg × 4px) sobre el título en el scrim slab, donde va el kicker de revista. Color-solo es curva SANCIONADA de la ley nunca-color-solo: el póster es aria-hidden (AT lee el pareo swatch+código+palabra de la cara densa) y la señal completa está a un hover. Paleta de suelo OSCURO (`categoryColor` de lib/utils, no `categoryColorOnLight`) porque la raya vive sobre el scrim de tinta. Alias conocido: los ámbar de reseña/artículo se confunden de reojo — aceptado (ambos son lecturas). Iker vetó la alternativa sello-de-esquina (chip swatch+código) pidiendo aprovechar el color-coding con sutileza.
- **Byline sellado (ronda 2)**: `CreatorChip` promovido de texto gris 9px a chip hairline de tinta (10px, hover = inversión de relleno) — LA gramática clicable del papel (GenreChipButton paper, chip de encuesta, TICKETS →). El byline libre `item.author` («por Redacción…») queda plano a propósito: identidad visitable vs crédito impreso ahora se VEN distintos. Prop `dim` eliminada.
- **Ronda 3 — código fuera + bug de barras anchas**: (a) TypeChip pierde la abreviatura («LI · LISTA» → «LISTA») — redundante junto a la palabra completa (Iker); el swatch queda. (b) **BUG REAL cazado por Iker**: MdCard elegía side-vs-top art por TIPO de contenido asumiendo MD_GEOMETRY, pero el pase SHAPE_CYCLE de rankItems (garantía de variedad anti-filas-de-cuadrados) reasigna CUALQUIER tipo a barras 2×1 — una lista/reseña en barra de 150px con layout top-art dejaba ~60px de leyenda y el título line-clampeado (min-content ≈ 0 por el -webkit-box) COLAPSABA a 0px mientras los chips de altura fija sobrevivían; el meter se estiraba a 760px. Fix: ContentGrid pasa `orientation` calculada del celle REAL (colSpan clampeado vs rowSpan, misma clamp que MosaicItem) → side art para ancho/cuadrado, top para alto; heurística por tipo sólo como fallback sin orientation. `cardArtSizes` también la usa (una lista en barra 2×1 ahora pide imagen a 66vw, no 33vw). Blindaje extra: `shrink-0` en los h2 sm/md — el título nunca es lo primero que se sacrifica en una leyenda sobre-restringida. Verificado: todas las barras 2×1 del feed (mix/lista/reseña) con side art + título visible.
- Verificado en dev (:53839, servidor propio de la sesión): tsc limpio, lint limpio, hover en md/lg, foco por teclado (opacity 0→1→0), móvil 375px (chips de encuesta + ★ visibles en reposo, títulos con clamp). `next build` NO corrido a propósito — dos dev servers sirviendo la carpeta. Sin commit: queda para el eyeball de Iker.

---

## 2026-08-30 · INGEST · «PLIEGO TOTAL» fases A–E — el rediseño site-wide construido y mergeado a `main` (`b943604`→`5029e5e`, fast-forward desde `17eabe9`)

Arco completo en una sesión: auditoría → mockups aprobados por Iker → cuatro fases construidas (multi-workflow, builders file-disjoint), cada una verificada (tsc · 66/66 dashboard · 64/64 mapa · `next build` limpio · walkthrough autenticado en dev) y apilada en ramas `pliego/fase-{a,b,c,e}`. Deliverables de diseño: auditoría «Pliego Total» + lienzo «Rediseño Gradiente» (artifacts de claude.ai, links en la memoria del asistente).

- **Fase A — fundación (`b943604`)**: CRT borrado entero (CRTOverlay/Shader/PostProcess; 1 contexto GL idle, era 2). Masthead de tinta 64px (SystemObject vivo, BUSCAR, chip AHORA, menú de identidad con VER PERFIL PÚBLICO ↗), colofón impreso. Player unificado en barra inferior faceplate (MarqueeText extraído a módulo propio — 3 importadores; QueueSeeder conserva el sembrado del HUD; ventana MATRIZ respeta visualizerSlot; NowPlayingHud borrado). `/api/search` real (auth-gated, allowlist, sanitizado) + SearchOverlay como hoja. Alturas sticky centralizadas: `--gr-nav-h`/`--gr-player-h` en globals. Fix del rename: toggle admin de marketplace + editar/borrar franja llamaban al endpoint muerto `/api/admin/partners/[id]`.
- **Fase B — el feed (`1fe31d3`)**: suelo de papel vía `html.paper-route` ([[Home]] + /agenda + 6 categorías; mapa en `lib/chrome/paperRoutes.ts` + `PaperGround` — mantener AMBOS en acuerdo). [[VibeSlider]] = banda espectrograma impresa (máquina de drag/detents/balística/chips portada verbatim; lectura SOLO PALABRAS). [[ContentCard]] re-vestido en 3 tiers: arte SIEMPRE presente (ley de Iker), leyenda fuera de la imagen, tintas `CATEGORY_ON_LIGHT` + códigos 2 letras, `VibeMeterLight` canónico, boleta de encuesta, `.print-fresh` (fuera-de-registro) reemplaza el kit fresh-glitch (borrado). PORTADA, rail AGENDA con ‹› reales, rail TIPO, FRANJAS·IDENTIDADES + MERCADO·CATÁLOGO como retículas. **EL CAMPO**: [[VibeFluid|Three.js Islands]] deja el viewport y se asienta como instrumento biselado en el rail (misma sim; gates movidos a `ElCampo`; resize por ResizeObserver+offsetWidth; sismógrafo en miniatura del viewport). Masthead/AuthBadge/AhoraChip/MobileNotice con doble estampa (papel = subrayado rojo · tinta = ácido).
- **Fase C — overlays (`ba8ed38`)**: [[OverlayShell]] = hoja sobre scrim de tinta (chip de tipo, sello //PRESENTA, CERRAR ESC; mueren eva-box/scanlines/phosphor/ONLINE y TODAS las filas de estado falsas). SignalTransition = revelado de medio tono (tinta sobre papel; contrato `onReveal` intacto). Lectores traducidos con anatomía propia ([[ReaderOverlay]] dossier · [[ArticuloOverlay]]/[[ListicleOverlay]] con §NN y «FIN» · [[MixOverlay]] funda de disco con visualizador en bisel · [[EventoOverlay]] con CTA rojo). [[VibeFader]] byte-intacto sobre faceplates negros. Comentarios en papel (badges chip-tinta+swatch; flash de foco rojo). PollSection boleta; PromptOverlay/PublishConfirmOverlay hojas; **HarvestConfirmModal reconstruido nativo → borrado el ataque CSS de globals** (la bomba `.z-[100].backdrop-blur-sm`). **MÓVIL: overlays ganan GUARDAR + COPIAR ENLACE** (barra 2×2, 44px) — hueco real cerrado.
- **Fase E — expediente (`5029e5e`)**: `/u/[username]` = expediente impreso (kicker rojo, banda de rango en la placa — glow de `avatarFrameStyle` retirado, chips tinta+swatch, firma impresa, VIBE DE SUS PIEZAS en palabras, TROFEOS con pictogramas SVG canónicos en `components/trophies/TrophyGlyphs` + FECHAS visibles; sigilos ASCII retirados; CERO números HP/HL; filas franja filtradas de PUBLICADOS por [[Franjas Isolation]]). Puertas: @username→Link en [[CommentList]] y [[PostHeader]] del foro, MdCard gana CreatorChip, [[Dashboard|IdentitySpine]] suma VER PERFIL PÚBLICO ↗. `/u/Rev.` (puntos) verificado en vivo.
- **Sistemas intactos (verificado por fase)**: curación/tiers/motion constitution, fricción del fader, contratos de URL (?item/?comment/?type&edit/?section), recordHpEvent + primado de itemsCache, leyes de audio (gesto síncrono, one-transport, slot único), authResolved, reduced-motion en paridad.
- **Sigue OSCURO a propósito (fase F)**: /foro (chrome propio; ojo: la banda de vibe es papel ahí — costura aceptada), /marketplace + overlays de mercado, [[FranjaOverlay]] (+ rama `partner/dossier-phase1` — coordinar), /mapa chrome, /admin, /espera, /about (fósil), /e/[slug], LivePreview. **Fase D pendiente**: pestañas PANEL·PUBLICAR·FRANJA·MERCADO — bloqueada en UNA decisión de Iker: gobernanza de `marketplace_enabled` (self-service vs aprobación admin; el código hoy implementa ambas historias).
- **Trampas nuevas para futuros pases**: la lista `PAPER_ROUTES` y los montajes de `<PaperGround/>` deben moverse JUNTOS; `ShareButton` conserva una variante 'dark' SOLO para FranjaOverlay (borrarla al convertirlo); RESEÑA reemplazó a REVIEW como etiqueta pública (canónica de palette.ts — avisado a Iker); el pane del navegador puede auto-cerrar un overlay z-60 recién montado con su click sintético (clicks DOM reales bien).
...
**Follow-up 3 — SHIPPED:** Iker approved; `main` fast-forwarded to the branch tip and pushed (`3ec23d8..01f7853`) — the merge also ships the branch's own work (485-entry taxonomy, foro custom tags, media embeds, listicle seeds). Verified pre-push: full `next build` on merged main (/welcome 12.9 kB) · 64/64 mapa · 66/66 dashboard. **Migration 0047 (foro_custom_tags) pending in the SQL editor — the deployed /api/foro/tags route needs it; 0046 still pending too.**
...
- **The editor**: ComposeSheet's interior rebuilt (file/exports/path intact — the compose-open garden-freeze signal, Esc-vs-publish-confirm guard, lab-aware close, z-60 all survive). Body = breadcrumb (DASHBOARD ◂ / NUEVO|BORRADORES / Tipo) · Syne H1 «Nuevo/Editar borrador / Mix» · HONEST autosave head («Se guarda solo» until the user actually edits — dirty-flag gated; then «Guardado automático · hace Ns» ticking, acid dot ≤30s) · numbered cream section cards (01 IDENTIDAD w/ side-by-side fields + quiet slug strip w/ lock + EDITAR SLUG ✎; per-type sections; COPY; VIBE+GÉNEROS — the two-thumb fader's drag machine VERBATIM (vibe-friction law) + SUGERIDO prior w/ real provenance; PORTADA dropzone via compressAndUploadImage; CONTEXTO/ENLACES; ENCUESTA) · sticky rail: **ESTADO DEL BORRADOR** (AUTOGUARDADO · COMPLETITUD n/m + acid bar + FALTA checklist rows that scroll to their fields — rules lifted 1:1 from the dark forms into compose/requiredFields.ts), **PUBLICACIÓN** (EDITORIAL now honestly staff-gated guide/admin — matches the /api/items rule, insiders no longer see a server-ignored lever; VINCULAR A MI PROMOTORA partner-gated; rows hidden never disabled), **ACCIONES** (GUARDAR BORRADOR / CONTINUAR DESPUÉS / acid PUBLICAR — gated w/ legible «FALTAN n CAMPOS», keeping the dark forms' guard-both coupling).
- **Architecture**: logic layer byte-untouched — useDraftWorkbench (its 3 shipped data-loss fixes reused, never reimplemented: edit-keyed sessionStorage slots, wait-for-cache ?edit hydration + explicit «CARGANDO BORRADOR…» state, URL-anchored publishMode), exact `gradiente:dashboard:<type>-draft` keys (PublishConfirmOverlay hardcodes the wipe pattern), publish recipe requestPublish→openConfirm (ONE confirm, never saveDraft-first), lib/drafts + caches, slugify, parseTracklistLine, poll helpers, EventoForm's attributePartner:true default. Complex editors forked light w/ verbatim logic under compose/kit/* (VibeFieldL, GenreMultiSelectL — now UNFOLDED into the page grouped by taxonomy root, no internal scroller; EntityMultiSelectL, EmbedListL w/ smart multi-URL paste, LinkListFieldL, StringListFieldL, PollFieldsetL, ImageFieldL) + the heavy suites (Articulo/Listicle block editors, MixTracklistEditor w/ the 3-format bulk-paste panel). **Dark forms/** stay byte-untouched — /admin still mounts them; LivePreview survives unimported on this path; the `.dash-press` compose reskin retired from globals.css (harvest-modal block intact).
- **True-data laws enforced** (deliberate omissions vs the mockup): no MP3/WAV dropzone (audio hosting doesn't exist), no B/I/MD toolbar (body is plain paragraphs), no Visibilidad select (one real value), no Programar (no scheduling), no LivePreview; pristine fields render CALM (errors touched-gated — the rail's FALTA list is the virgin-page signal); no demo-content placeholders (format hints only); rail never renders publishedAt for drafts.
- **Judge round 6**: 8/10 → fixes: genre picker unfolded (the page is the scroller), pristine-scold killed, autosave honesty, placeholder sweep, BORRADORES crumb closes + scrolls to CULTIVAR, excerpt register unified, FECHAS browser-format hint, APLICAR/vibe-thumb hit targets, H1 sentence-case type (COMPOSE_TYPE_DISPLAY beside the untouched chip map), stale CSS comment.
- Verified: tsc · 66 tests · live walkthrough (checklist scroll-to-field, live COMPLETITUD, slug toggle, CERRAR-in-place on the lab). **Pane note for the quirks memory: the tab compositor FROZE twice this session (screenshots identical across scroll/resize/reload while DOM tools kept working) — cure = tabs_create a fresh tab; resize emulation can corrupt the capture surface.**

---

## 2026-08-21 · INGEST · «EL PLIEGO» scale pass — fixed portions, generous items, no hidden scrollers · `e7cd421` (pushed)

Iker's same-day review of `b8aad38` against his reference mockup: "many options are behind very small scroll options and key buttons/cards/details are too small." Root cause: the build had solved its height budgets by COMPRESSING content into pills and hidden-scrollbar rails — the opposite of the reference's law. The scale pass (SCALE_PASS.md, session scratchpad) makes that law binding: **at default size every widget renders a FIXED number of COMPLETE, generous items — overflow lives behind ONE shared `VerRow` affordance; internal scrolling exists only in explicitly-chosen large/expanded states (S1)** + minimum scales (rows ≥52px, 48px thumbs, buttons ≥36px, covers ≥140px — S2/S3).

- New defaults: GUARDADOS {7,3} = a 4-cover art gallery (VER TODO commits {12,3}, where the snap-rail returns); REPRODUCTOR/NOVEDADES {5,3} thumbed rows; AGENDA {4,3} leads with a reference-style PHOTO CARD (flyer + category chip + d18 title + big acid date block); MAPA {3,3}; ACTIVIDAD = 5 avatar'd rows + MOSTRAR ANTERIORES expansion; CULTIVAR h4 band — w-80 acid slab (36px chips), garden ≥171px, COSECHAR pills TWO-LINE (title owns line 1 whole; portion 2/{8,4}, 3/{12,4}, 1/h3) with garden-click focus-slot navigation + a «+N EN EL JARDÍN» declaration, CONTINUAR = 2 boxed rows + expand. WidgetFrame p-5, standardized 32px header line, 44px hit pads on header actions; zero-count facets hidden at default (the strip fits whole); trophy strip is trophies-only (the spine alone owns PRÓXIMO HITO — the echo could never render whole and died rather than truncate). Layout meta **v:3** — all pre-pass layouts normalize to the new defaults.
- Verified: tsc · 66 tests · judge round 5 at exactly 1440×900 (generosity criteria; its one fatal — 27px pill titles — fixed by the two-line pill geometry, seals 92px unclipped, titles measured whole in DOM).
- **Prod hygiene for Iker:** a fixer-team test draft («Test publish for verify») sits in the real BORRADORES next to the old «asd» draft — delete both by hand when convenient. Accepted budget deviations on record: AGENDA secondary rows 40px under the lead card; MERCADO {6,2} = 2 side-by-side whole cells (VER TODO grows it); REPRODUCTOR/NOVEDADES portions drop one deterministically when the transport is active / feed overflows.

---

## 2026-08-21 · INGEST · «EL PLIEGO» — /dashboard rebuilt as a light widget-grid instrument panel · branch `dashboard/aaa-revamp` (`b8aad38`, pushed)

Iker's directive: rebuild `/dashboard` at "AAA social media" quality — modular snap-grid widgets, per-user customization, a LIGHT high-contrast editorial language (his mockup: cream paper, ink hairlines, black anchor panels, one acid accent), creation friction killed, every live system respected. Built via multi-workflow orchestration (13-reader recon → 5 harsh critics → 3 competing designs → 3 judges → synthesis → staged file-disjoint builders → 3 rounds of a browser-driving AAA judge). **Judge verdicts: 7/10 → 8/10 → 10/10 SHIP on the wow criteria.** 83 files, +13,351/−8,590, `next build` clean, NOT merged to main (Iker's eyeball is the gate).

- **The design:** full-bleed paper shell (`fixed inset-0` — ChromeFrame/CRTOverlay/GlobalPlayerBar all null on `/dashboard` + `/lab/dashboard`), 48px black masthead, IDENTITY SPINE (Syne display greeting + the ONLY raw HP scalar, PRIVADO-framed, + status strip that collapses at zero), then a 12-col/96px grid of 9 widgets: **CULTIVAR {8,4}** (acid CREAR slab w/ always-visible role-gated type chips = 1-click compose; two-line CONTINUAR drafts; MIS PUBLICACIONES w/ the **JARDÍN DE SEÑAL** — a three.js HP-decay garden, masses ∝ `currentHp()`, FFT-rippled while audio plays, canvas-2D GardenFallback, ≤2 WebGL contexts via visualizerSlot; COSECHAR pills reusing [[HarvestConfirmModal]] byte-untouched + the synchronous broken-seal cache patch; SVG trophy pictographs), **ACTIVIDAD {4,4}** (derived inbox: comments-on-my-items / replies / reactions / foro / LOGRO + partner OFERTA fold-in — all prod-validated queries; ONE localStorage watermark shared w/ the spine; ruled-ledger sparse state), **GUARDADOS** (one faceted collection, true saved_at via the Map-upgraded itemSavesCache, bottom-gradient art tiles, QUITAR+undo through toggleSavedItem only), **REPRODUCTOR** (playQueue-in-gesture on the real AudioPlayerProvider, honest «N REPRODUCIBLES · N SOLO FUENTE», MiniTransport pin at z-45), **NOVEDADES** (localStorage follows, picker-as-empty-state), **AGENDA** (global fallback — never empty), **MAPA** (SVG CdmxSchematic, dots ONLY via the curated venueGeo table, SIN UBICACIÓN honesty), **PERFIL** (document-at-rest direct-edit; VIBE PERSONAL words-only from own vibe_checks, «SEÑAL INSUFICIENTE · N/5»), **MERCADO** (partner-team/admin only). Compose = the 8 forms byte-untouched inside ComposeSheet's press bed, reskinned via scoped CSS (`.dash-press` + `html.dash-route` in globals.css) — the orange EVA chrome → ink/acid; publish keeps exactly ONE confirm.
- **Grid mechanics:** in-house, zero new deps — deterministic packer (lib/dashboard/layout.ts), edit-mode-only drag (6px threshold, hard-offset shadow, live position-only reflow, dashed ghost, no springs/rotation), resize snaps through per-widget allowedSizes, OCULTOS tray w/ live counts, RESTABLECER (epoch-keyed remount). Persistence: layout ONLY in `profile_meta.dashboard` (**v:2** — v:1 was the pre-ship judging schema and normalizes to defaults; server does key-by-key namespace merge in /api/users/me, RLS-verified self-updatable) + localStorage mirror; **follows + lastSeen watermark are localStorage per-uid** (private-class like saves — profile_meta is member-readable). Mobile = 1-col priority stack, 44px targets, drag desktop-only.
- **Systems integrity (all verified):** zero SQL; /api/items, HarvestConfirmModal, PublishConfirmOverlay, AudioPlayerProvider, forms/**, categoryColor/VIBE_SLOT_COLORS byte-untouched; ONE DashboardDataProvider — runtime ledger measured on prod data: one 60s poll burst (8 requests: engagement + the inbox footprint), ZERO realtime channels from dashboard code, one profile_meta write path; saves emit HP through lib/saves.ts only; no engagement metrics, no numeric vibe anywhere; every readout traced to a real query. Legacy `?section=` URLs resolve (scroll/facet map); `/dashboard/drafts` redirect + OverlayShell EDITAR deep links intact. Deleted after porting: the whole explorer tree + PlaceholderForm/DraftsList/TypePicker (EngagementWidget math → spine/TrophyStrip, ProfileSection → PerfilWidget, MiPartner/Approvals → MercadoWidget). LivePreview survives (forms).
- **Dev harness:** `/lab/dashboard` (dev-only middleware gate) renders the REAL shell+grid against 6 fixture scenarios (fresh/rich/partner/admin/smallN/error) injected ONLY through the provider `initialSlices` + shell `userOverride` doors; compose mounts on the lab too (never ejects to prod); fixture artwork = committed flyers only; ids uuid-shaped.
- **New tests:** tests/dashboard/layout.test.ts (32) + contrast.test.ts (34 — §1.5 category-on-light ≥4.5:1, four spec hexes darkened per its own rule; hlBracket parity with both legacy copies). Run `npx tsx --test tests/dashboard/*.test.ts`.
- **Traps recorded:** tailwind.config.ts edits need a dev-server RESTART (JIT under HMR never emitted the new tokens — cost a full judging round on a token-less page); the browser pane's injected touch tap hangs ~30s on scroll-locked dialogs (pane quirk — synthetic pointer events fine; MobileNotice hardened w/ onPointerUp anyway); PowerShell Get/Set-Content mojibakes UTF-8 (memory `no-powershell-text-roundtrips`).
- **Judge residuals (non-blockers):** real-device mobile tap test before mobile comms; longest CONTINUAR titles still shave characters at 178px (cosmetic); category swatches remain the site's stock hues (2-letter codes carry distinguishability — a tuned print-ink set is a future cosmetics pass); unequal-band packer gap beside compact strips accepted as spec-inherent; frozen modal's Loader2 spins mid-submit (needs a modal rebuild to fix).
- **Iker's calls before merge:** eyeball the garden on real hardware (the R2 craft gate) + the whole surface at real fps; venueGeo `Loo Loo` (San Rafael) + `Foro Bizarro` (Obrera) placements are neighborhood-confidence — one-literal fixes in lib/dashboard/venueGeo.ts; whether `/lab/dashboard` should also gate behind auth in dev. The type merge (articulo/editorial/opinion) deliberately NOT done — single edit point is COMPOSE_TYPE_LABELS in CrearZone + typeDisplayLabel in lib/dashboard/palette.ts (GUARDADOS already folds the three into its ARTÍCULOS facet).

---

## 2026-08-20 · INGEST · `/mapa` pass 15 — identity rosette, obi redesign, MERCADO kill-switch, cell-text containment · [[Mapa Placement Rules]]

Iker's four-point review of pass 14, all landed — committed + pushed to `main` on 2026-08-21, in the same commit as pass 14:

- **Identity rosette** — the focus nucleus was a single hex ("hard to access from the map view"). [focus.ts](../lib/mapa/focus.ts) now claims a full 7-cell rosette (`identityCells` on FocusArrangement; lens variant stays chrome-less) and the [MapaCanvas](../components/mapa/MapaCanvas.tsx) nucleus renders like a dominant slab: the partner image FILLS the rosette (SmartImage + gradient), with a `//PARTNER · KIND` chip and a 48px Syne wordmark centered over it, heavier white rim. Members/listings/belt/displacement all flow around it via the existing focusOwner machinery. New test pins the rosette shape; the reserve/occupancy tests moved from `identityCell` to `identityCells`.
- **Obi redesign** — [PartnerObi](../components/mapa/PartnerObi.tsx) rebuilt to the Club Japan obi reference: large logo up top, dominant VERTICAL Syne wordmark flanked by ornamental katakana (per-partner map: club-japan/noche-negra/naafi + partner-kind katakana fallback so every partner wears the accent) and a red hanko-style seal; info block with horizontal name, address split to lines, **contextual per-kind rows from real member data** (venue/club → HORARIO SEGÚN EVENTO; promoter/colectivo/festival → PRÓXIMA or ÚLTIMA fecha from the cluster's eventos; marketplace-enabled → n ARTÍCULOS EN VENTA; always EN EL MAPA count), @handle link, and a **social icon row** derived from `ContentItem.links` + partnerUrl (IG/X/YT/FB lucide icons, SC/BC/MC letter chips, globe fallback — dedup by URL, nothing invented; NN partner row gained its two real links: IG + Facebook). Mobile sheet keeps a horizontal variant. Canvas passes the cluster's member items (`focusMemberItems`).
- **MERCADO kill-switch** — the filter column lacked marketplace. New warm-sand (#D6B37A) MERCADO hex after the type hexes (renders only when a clustered partner actually sells — honest chip; dev seed shows 11 = NN 8 + CJ 3), key `'mercado'` in the hidden set + `?ocultar=mercado` boot restore; [MapaListingCell](../components/mapa/MapaListingCell.tsx) gained the `hidden` fade-in-place contract (aria-hidden, tabIndex −1, `.mapa-cell--off`).
- **Cell-text containment** — titles overflowed the tapering hex corners and overlapped neighbors. MapaCell + MapaListingCell text blocks are now CLIPPED to the same hex path as the media and centered on the shape's widest band (chip top-center, title+meta centered, line-clamp + truncate) — text can no longer leave its slab. Follow-up (Iker caught trihexes cropping their type chip): the chip anchors to the polyhex's TOPMOST CELL rather than the bbox top-center — a trihex's bbox top-center can fall on the notch between cells where the clip crops it; single hexes keep the identical position by construction (offset = 0.84·half-height above the cell center). Verified with a Path2D `isPointInPath` sweep over all 248 live cells: zero cropped chips on any template.
- **Global MERCADO satellites (same-day follow-up)** — Iker: "when I filter everything except MERCADO, it doesn't show a single marketplace cell, why?" Because listing nodes only existed in focus (the deferred open item). Built `placeGlobalListings` ([focus.ts](../lib/mapa/focus.ts)): every marketplace partner's listings become single-hex satellites BFS-placed on the free cells nearest its cluster — never displacing the stable terrain (rule 9; the interspersed center has no free neighbors, so they take the nearest coast and chain into an arc). Each satellite records its nearest member as ANCHOR and follows that member's delta (rides the continent drift + focus displacement), dims during another partner's focus, hands over to the focus arc during its own, fades during compaction, and obeys the MERCADO kill-switch — the mercado-only filter now shows exactly the marketplace layer (11 nodes on dev seed) over the faded terrain. Listing detail opens from global satellites too (`openListing` generalized to (listing, partnerSlug); `?partner=&listing=` deep link no longer requires `?focus=`). Rules-note open item closed. +3 tests (determinism/no-overlap, near-cluster + real anchors, skips non-marketplace partners).
- **LISTA → magenta `#EC4899`** (Iker's call after the ΔE analysis) — review↔listicle sat at ΔE2000 9.4 and listicle↔sys-orange chrome at 7.7; three categories plus chrome, MERCADO sand and the vibe hot arm were sharing one warm sector. Magenta was the system's only vacant hue family (nearest neighbor: opinión violet at ΔE 25.8; 5.5:1 on the dark ground; CVD-separable from evento's red via the blue channel). One-line change in `categoryColor` — every surface (cards, rail, chips, map rims, filter hex, ListicleOverlay eyebrow) follows; [[Color System]] + [[ListicleOverlay]] notes updated. Watch-items recorded, no action: review↔mercado-sand 13.8 (both are map rims, but sand is desaturated + chipped), review/vibe-9 ~5 (vibe colors never appear as cell chrome).
- **Verified**: `tsc` · 64/64 tests (rosette + 3 satellite tests) · live on :3003 — rosette 550px wide with image fill + 48px wordmark; obi shows ノーチェ・ネグラ + 2 social chips + ÚLTIMA/MERCADO/EN EL MAPA rows; MERCADO toggle fades all 8 listing nodes with `?ocultar=mercado` and restores; cell text clip+center confirmed in DOM. Prod build ran clean on 2026-08-21 in a quiet window (`/mapa` 22.2 kB) right before the commit.

---

## 2026-08-20 · INGEST · `/mapa` pass 14 — AFINIDAD continents mode + the Noche Negra partner catalogue (25-item seed) · [[Mapa Placement Rules]]

Two Iker directives: (1) an affinity FILTER that hides nothing — it identifies the major high-affinity areas and grows the padding between everything else, "continents of mind map"; (2) a full mock catalogue for **Noche Negra** (real CDMX vinyl-only tropical collective, IG @nochenegra.mx) showing what a mature partner focus looks like months/years in. Both BUILT + verified; committed + pushed to `main` on 2026-08-21, in the same commit as pass 15.

- **AFINIDAD mode** — new [lib/mapa/continents.ts](../lib/mapa/continents.ts) (rule 12 in [[Mapa Placement Rules]]): union-find over ADJACENT item pairs whose affinity clears a threshold → landmasses; each mass drifts rigidly from the terrain centroid (spread 1.5) with nearest-fit ring-search collision repair and per-class oceans (continents 2 rings, islets 1). The load-bearing find: a fixed threshold **percolates** — dev seed at the floor (2) fuses one 150-item Pangaea — so the threshold ESCALATES until the largest mass ≤35% of terrain (dev lands at t=5: masses 26/25/22/7/6/6…), with back-off if escalation over-fragments. Deterministic, translation-only, reversible, ~6ms at dev scale. Chrome: opt-in AFINIDAD hex under the era kill-switches (sys-orange, live continent count, no strike-through when off — `strike` prop on ToggleHex), dashed perimeter rings fade in behind the 700ms drift, `?afinidad=1` via replaceState, camera clamp/fit follow the expanded bounds. Precedence chain: focus > continents > compaction > global — verified live: focus suspends the drift and unfocus restores it; hiding a category while active fades in place (no compaction).
- **Noche Negra catalogue** — 24 content items + 1 partner row appended to [lib/mockData.ts](../lib/mockData.ts), ALL stamped `partnerId: "pa-noche-negra"` (mirrors the prod partner row id): **6 eventos** (Sonidos por Venezuela benefit Jul 24 2026 at Salón Los Ángeles — 11-act lineup from Marvin's coverage; Tonal Oct 2025 w/ real RA flyer; Agenda Tonal Apr 2025; NYE Bienvenido 2026 at Salón Solín; El Baile del Año Nov 2024 sonidero summit; Club Coco x Nochenegra Nov 2024), **5 mixes** (Carlos René "Salsa en México" NTS/Mixcloud · Coco María Le Mellotron · Sonido Martines "El Humo Mundial" · Sonido Confirmación RA sonidero mix · Tropicaza Boiler Room CDMX — every URL verified live, tracklists where published), **3 noticias** (Sonidos recap — Marvin summarized in OUR words + linked, full text deliberately not copied (press copyright); SXSW 2024; siete años), **4 reviews** (all three Club Coco volumes incl. the 2025 self-released "New Dimensions", + the Tropicaza-curated Vampisoul reissue "La Onda de Elia y Elizabeth" — found inline after the Tropicaza research agent died twice on a spend-limit error), **3 opiniones** (más salsa en la noche · elogio del salón · vinyl-only como memoria), **3 listicles** with `track` blocks — 15 tracks ALL YouTube-verified with i.ytimg thumbs (subgéneros raros de la salsa / boogaloo-descarga / cumbias de salón). Partner row: marketplace-enabled with **8 listings** (SL-1200MK2/1210MK2/MK5 · Shure M44-7 + N44-7 NOS (2018 discontinuation story) · Technics SFWE010 puck · Mukatsuku steel domes · vintage bullet lote) at researched MXN prices.
- **Research** — an 8-agent web workflow (marvin/RA/collective recon/Coco María/Tropicaza/mixes/products/listicle tracks) with per-agent link verification; 6/8 landed (RA-events + Tropicaza deep-dive died on a spend-limit error — their facts were covered by the other agents' overlap). IG `web_profile_info` worked from this machine: live bio (52.5K followers) + pfp + 6 recent post images pulled and rehosted. **32 images downloaded to `public/flyers/nn-*`** (Marvin S3, images.ra.co, Time Out, Bandcamp f4.bcbits, SoundCloud/NTS CDNs, Wikimedia — the 429s needed slow sequential retries), sharp-compressed ≥400KB → ≤460KB each. No item without an image (rule 1 holds).
- **Genres/tags** — all resolvable through [[genres]] taxonomy (salsa/cumbia/cumbia-sonidera/sonidero/latin-jazz/nu-cumbia/…), so the catalogue carries REAL affinity signal: with it, the dev terrain (256 items) partitions into **7 continents** — the tropical mass among them — and partner focus gathers 23 members + nucleus + 8 MERCADO nodes (verified live: selector "Noche Negra · 23", listing hex → MarketplaceListingDetail `?listing=mkl-nn-01`, listicle overlay renders track blocks + partner attribution).
- **Verified**: 60/60 tests (6 new in [tests/mapa/continents.test.ts](../tests/mapa/continents.test.ts): determinism, community separation under escalation, ocean guarantees, rigid drift px≡axial, bounds growth, null cases) · `tsc` · lint · prod build (`/mapa` 17.5 kB) · live walkthrough on :3003. Pane needed the stale-chunk cure (`fetch cache:'reload'` + reload) once before hydration came alive.
- **Same-day correction (Iker's review)** — "Noche Negra doesn't appear under partners." Cause: his session was AUTHED, so /mapa read the live DB (which also holds the 160 legacy seed rows — the "old mock data" he saw was prod content, not something added) and the file-side catalogue never reached the view; only anon sessions (MOCK_ITEMS fallback) showed it. Fix: catalogue extracted to its own module [lib/nocheNegraSeed.ts](../lib/nocheNegraSeed.ts) (`NOCHE_NEGRA_ITEMS` / `NOCHE_NEGRA_PARTNER` / `NOCHE_NEGRA_SEED`; mockData spreads it so every other surface is unchanged), and [app/mapa/page.tsx](../app/mapa/page.tsx) now merges it in DEV builds whenever the fetched dataset has no NN-attributed content — replacing the DB's bare partner row with the marketplace-enabled demo row. Prod never shows the mock catalogue (`NODE_ENV` gate). Verified: selector shows "Noche Negra · 24" with nucleus + 8 MERCADO nodes; tsc + 60/60 unchanged.
- **Open**: continent labels (what do these share? — a "dominant trait" chip per ring would explain the geography); AFINIDAD ∧ ocultar currently fades hidden cells in place — compacting *within* continents is a possible later composition; spread/ocean/share-cap tunables await Iker's eyeball on real hardware; whether the NN demo should also merge on PROD /mapa for a team demo is Iker's call (one-line gate change).
...
- **Synthetic HL injection** — real HP is flat until the deferred hp_events writer ships, so the terrain read as a field of singles (dev 219×1/10×3/4×7). New `sizeTiers`/`placeItems` option `{syntheticHl}` ([layout.ts](../lib/mapa/layout.ts), page passes `SYNTHETIC_HL=true`): deterministic id-hashed promotions (avalanche hash, seeded differently from bearings so size never correlates with direction; ≈5.5%→rosette-candidates, next ≈28%→trihex, archive era at HALF rate so rim stays quieter than center, cap 8). **Two-pass contract** (post-review): real tiers assign FIRST and claim dominant slots, synthetic spends only the leftover budget — promotion-only, plain eventos still never rosette, same layout for every viewer. Dev terrain → **154×1/71×3/8×7**; rule 2 amended in [[Mapa Placement Rules]] with the retirement note. Tests: default-off equality, determinism, never-demotes, caps, archive damp, + the stale-peak-mix regression corpus.
- **Packed-grid engine rewrite** — a benchmark subagent profiled the greedy walk at archive scale first: 2,400 items = **4.7s**, O(n^1.5), 95% in the candidate loop — ~27.6M string `cellKey` builds, and the string-pair affinity memo's lookup overhead costing ~3× the affinity computation it saved. Rewrote `placeItems` + `compactLayout` hot paths onto **integer-packed cells** (`pack(q,r)=(q+2048)·4096+(r+2048)` — additive against raw deltas so template offsets/hex dirs become plain adds; numeric key order ≡ old (q,r) lexicographic sort, so walk order and float-accumulation order are unchanged), generation-stamped per-item affinity arrays replacing the global memo, decorated prominence sort. **Byte-identical verified** two ways (scratchpad goldens both modes; independent old-vs-new parity harness at n=50/233/600/1200 incl. compact deltas) and now PINNED by a committed **layout-fingerprint test** (sha256 of serialized geometry, both modes — intentional tuning updates it knowingly). Result: **2,400 items 4.7s → 1.10s (4.3×)**, dev seed 151→59ms. Next lever if the corpus outgrows ~5k: bounded frontier window (changes output — needs its own quality review; recorded in the rules note).
- **Imagery through the optimizer** — [[MapaCell]]/[[MapaListingCell]] now render via the existing `SmartImage` (per-size `sizes` hints; GIFs/unknown hosts stay raw `<img>` so flyer GIFs animate). `web.archive.org` (**`pathname: '/web/**'` — load-bearing**: hostname-only would make `/_next/image` an open proxy for the whole archived web incl. Wayback's state-changing `/save/` endpoint; verified live: `/web/` 200, `/save/` 400) + `i.ytimg.com` added to remotePatterns + `OPTIMIZABLE_HOSTS` — Wayback scans arrive resized/WebP/edge-cached (1-yr TTL). SmartImage went `'use client'` with an **optimizer-error fallback to the raw URL** remembered in a module-level `failedSrcs` set — per-mount state alone would re-blank fallback'd cells on every virtualization remount and hammer the rate-limited origin (review find). Blast radius checked: sole server-component consumer (MarketplaceRail) passes serializable props only.
- **Interactions/animations** — (1) **boot entrance ripple**: center-out staggered cell reveal (CSS animation on a new inner wrapper — the root transform belongs to focus/compaction deltas; `--mapa-enter-delay` from plane-distance; `.mapa-booting` scoped, dropped at 1.2s AND on first interaction so mid-boot pans never mount invisible cells; imperative classList like `.mapa-zooming` so React's constant className never wipes gesture classes). (2) **Hover press**: image scales 1.045 inside the hex clip + rim brightens to full — compositor-only. (3) **Cursors**: root `cursor-grab`, `.mapa-dragging` (class-toggled off the render path) → grabbing + cell pointer-events suspended mid-pan. (4) **Double-click zoom-at-point** (same anchor math as ctrl-wheel, verified via synthetic wheel) — scoped to grout/empty terrain only: cells/listings/nucleus own their clicks (new `data-mapa-node` on the non-item nodes); `zoomStepTarget` synced so +/- stacks correctly during it. (5) **Far-band declutter**: trihex titles now hide at far too (70 trihexes of 6px smudge post-injection); rosette titles remain. All new motion in the reduced-motion + `.mapa-zooming` suspension blocks.
- **Ultracode review** — 4-dimension workflow (engine/react/css/blast), adversarial verify per finding: 8 confirmed → all fixed this session (incl. the two majors: synthetic-demotes-real-dominant, Wayback open-proxy), 1 refuted (pointer-capture retargets :hover during pan, so the listing-cell hover-suspension "gap" can't fire).
- **Verified**: 54/54 tests · `tsc` · lint · prod build (`/mapa` 15.9 kB) · live: terrain texture, hover, compaction round-trip, focus deep-link, filter URL. **Pane earned four new quirk entries** (memory `browser-pane-quirks`): hidden tab ⇒ lazy images never load / transitions+rAF frozen / 0-viewport boots; `resize_window` sometimes restores. Frame-feel + ripple/camera easing still need **Iker's real machine**.

---

## 2026-08-18 · INGEST · `/mapa` pass 12 — COMPACTION: the terrain closes ranks when categories are hidden · [[Mapa Placement Rules]]

Iker: pass 11 was missing the positional restructuring upon filtering. Built `compactLayout` ([layout.ts](../lib/mapa/layout.ts)) + wired the delta chain (focus reflow > filter compaction > global).

- **The instructive dead end, measured**: the first version re-ran the placement with IDENTICAL weights over the visible subset — and produced ZERO deltas on the full dev dataset (131 events hidden, 102 visible, 0 items isolated). Cause: rule 6's interspersion keeps visible content mutually adjacent, so affinity re-wins every original spot; the faithful re-run is a no-op. Compaction therefore uses **packing-dominant weights** (`W_RADIAL_COMPACT 1.1` vs 0.02, `W_CONTACT_COMPACT 0.7`, `W_ANCHOR_COMPACT 1.4` so mindshare bearings hold) — affinity still arranges who sits next to whom while the mass tightens. Gated: null when nothing is hidden (a filtered VIEW is allowed to trade global stability for tightness; the global map never re-packs).
- Shapes frozen ⇒ translation-only glide (the existing 700ms transform transition animates both directions); hidden cells fade in place (no delta); derived layout feeds keyboard nav; cached per hidden-combination; suspended during partner focus. ~16ms per combination at dev scale.
- **Results**: events hidden → 74/102 items move, visible bounds shrink 3025×2572 → ~2530×2477, archive band pulls in 12.7 → 10.6 HEX_R while contemporary core holds at 7.2 (rim-is-memory survives compaction); compacted mass is one connected blob. Events+archive hidden → 58 survivors close into a single tight mass (verified live, 31 mounted movers).
- **Known tail** (recorded in rule 11): Club Japan's 7 visible members fragment under hard packing (max nearest-member gap 7 hexes) — their hidden eventos were the connective tissue. Focus-gather is unaffected; a same-partner cohesion term is the named fix if it matters in practice.
- **Tests 47/47** (replaced the zero-delta identity assertion with: null-when-nothing-hidden + movement + spread-shrinks metric + connectivity + frozen shapes + hidden-never-in-cell-index) · `tsc` · lint · prod build clean (`/mapa` 15.4 kB).
- **Pane note**: dev-server restarts leave the browser pane serving cached chunks from the DEAD server (404s + the recurring phantom SyntaxError, hydration silently dead) — `fetch(src, {cache:'reload'})` + reload cures it; added to the browser-pane-quirks memory pattern.

---

## 2026-08-18 · INGEST · `/mapa` pass 11 — visibility column: filters inverted to category kill-switches · [[Mapa Placement Rules]]

Iker inverted the filter model: instead of the corner radial that SELECTS a filter, a right-edge COLUMN of hexagonal toggles where clicking DEACTIVATES a category from the map. Rule 11 rewritten (filters are exclusions, never rebuilds).

- **[MapaFilterColumn](../components/mapa/MapaFilterColumn.tsx)** — vertically-centered right-edge column: 8 type hexes (category-colored, live counts) + separator + AHORA / ARCHIVO era hexes. Everything visible by default; a deactivated hex goes hollow (0.38 opacity, struck label) and its cells fade **in place** (`.mapa-cell--off`: opacity 0, pointer-events none, aria-hidden, tabIndex −1 — the transform/opacity transition animates both directions). Nothing moves; the geography holds.
- **Keyboard**: arrow navigation walks past hidden cells (bounded 8-hop scan in `onArrow`) so focus never lands on an invisible hex.
- **URL**: `?ocultar=evento,era:archivo` (replaceState; boot-restored, key-validated). The `?filtro=/modo=` lens params are gone.
- **Removed**: MapaFilterRadial (pass-10 UI, lived one pass — the corner fan is preserved in git history/log if ever wanted), canvas lens state (lens/lensMode/applyLens/lensArrangement/genreOptions). The GATHER ENGINE stays: `computeLensArrangement` remains exported + tested (partner focus shares its core; a future COMPACTAR button could re-surface it). Genre visibility toggles deliberately NOT in the column (18 hexes don't fit; genre-level control is an open question — per-root toggles would need a second column or a grouped hex).
- **Verified**: 44/44 · `tsc` · lint · prod build clean (`/mapa` 14.5 kB) · live: EVENTO toggle → 129 cells fade in place + hex goes hollow + `?ocultar=evento`; deep link `?ocultar=evento,era:archivo` boots with both hexes deactivated ("Mostrar EVENTO (131)" / "Mostrar era ARCHIVO (44)").

---

## 2026-08-18 · INGEST · `/mapa` pass 10 — radial hex filter (corner UI replaces the dropdown panel) · [[Mapa Placement Rules]]

Iker: the FILTRO dropdown covered the map — make it a radial UI with hexagonal icons at the bottom corner. Presentation-only pass; lens semantics (rule 11) unchanged.

- **[MapaFilterRadial](../components/mapa/MapaFilterRadial.tsx)** — a honeycomb of controls for the honeycomb: a hex FAB at bottom-right (shows the active lens + mode); tapping fans an inner arc of KIND hexes (TIPO · GÉNERO · ERA · MODO toggle · ✕ LIMPIAR when active); picking a kind fans a staggered double arc of VALUE hexes (up to ~9 fit the corner quadrant), each with its count — tipo values wear their `categoryColor`, era ARCHIVO wears the aged-paper rim color. Hex shape via clip-path + stacked-hex border trick; staggered fade-in (animation-fill-mode backwards); rectangular focus rings for keyboard users. Everything hugs the corner semi-transparent — the terrain never sits behind a panel.
- The old dropdown panel is deleted; the keyboard legend moved to bottom-left (the radial owns bottom-right). Terrain tap / ESC still collapse; deep links unchanged.
- **Gotcha caught live**: the corner-anchored offset math had an inverted `right` sign — value hexes stacked off the right edge; the arc only reads correctly with `right: x - size/2` (x grows leftward from the corner anchor).
- **Verified**: 44/44 · `tsc` · lint · build clean (`/mapa` 15.6 kB) · live: FAB → kind arc → TIPO value fan (9 type-colored hexes with counts) → EVENTO applied (102/231 dimmed, FAB shows EVENTO·DEST), map visible throughout.

---

## 2026-08-18 · INGEST · `/mapa` pass 9 — LENSES: filter by tipo/género/era with DESTACAR + REAGRUPAR · [[Mapa Placement Rules]]

Iker: filter the honeycomb by categories/affinities/related-unrelated and RE-STRUCTURE per filter. Design call codified as **rule 11 — filters are lenses, never rebuilds**: the stable shared geography (rule 9) survives filtering because a lens changes what you see, not where things live.

- **Engine** — [focus.ts](../lib/mapa/focus.ts) refactored: the gather/belt/displacement core is now `gatherArrangement(layout, memberIds, anchor, {nucleus, listings})`; partner focus = nucleus variant (unchanged, tests prove it); new `computeLensArrangement(layout, memberIds)` = nucleus-less variant — the first matching item seeds at the set's own centroid, members gather in type-ordered arcs, the affinity belt rings them, the rest dims. Translation-only + deterministic + reversible, same as focus.
- **Chrome** — `◐ FILTRO` panel beside PARTNERS: mode row [DESTACAR | REAGRUPAR] + `//TIPO` (types present, with counts) + `//GÉNERO` (top-8 taxonomy ROOTS present — matching runs through `itemMatchesGenreFilter` rollup so legacy ids count) + `//ERA` (AHORA / ARCHIVO 2005-2013) + LIMPIAR. Honest chips: only values that match something are listed. Lens ↔ focus exclusive (either may be active, never both); ESC clears; terrain tap dismisses the panel; REAGRUPAR flies the camera to the gathered set (obi space not reserved — no identity strip on a topic).
- **URL** — `?filtro=kind:value&modo=destacar|agrupar` via replaceState (deep-linkable; no history spam — Back-unwind for lenses deliberately not implemented, focus keeps its pushState/Back contract). Boot restores the lens; `?focus` wins when both present.
- **Tests 44/44** (new: lens gathers a topic set contiguously with the anchor owned by a MEMBER (no identity chrome), determinism; all focus tests unchanged through the refactor) · `tsc` · lint · prod build clean (`/mapa` 14.8 kB).
- **Verified live**: interactive TIPO lens dims correctly; deep link `?filtro=genero:techno&modo=agrupar` boots into the gathered techno mass + belt over a dimmed field. One debugging note: a fresh dev server can serve the page compiled BEFORE the latest edits on its very first request — a deep-link "bug" that vanished on the next load; check the served chunk before diagnosing state logic.
- **Open**: vibe-range as a lens axis (the site's core filter — VibeSlider chrome is nulled on /mapa; wiring VibeContext in is its own design question), lens+lens composition (e.g. tipo∧género), Back-unwind for lens history.

---

## 2026-08-18 · INGEST · `/mapa` pass 8 — Archivo Vivo on the map, image-only terrain, HL variation · [[Mapa Placement Rules]]

Iker: populate the map with the gradiente-ops **living-archive-pilot** (Altered Zones 2005-2013, `~/Documents/gradiente-ops-main` branch `living-archive-pilot`), remove imageless cells, and vary HL for structure. Read MANUAL.md in full first (it's the Supabase handoff addressed to dev).

- **Converter** — [scripts/buildArchiveSeed.ts](../scripts/buildArchiveSeed.ts): deterministic, re-runnable, no model calls (the archive's own tools posture). Reads az_posts.json + entities/post_entities CSVs → emits [lib/data/archiveSeed.json](../lib/data/archiveSeed.json). Honors MANUAL: visible credit (author blog + original year on the CELL; blog + Wayback links in CONTEXTO), excerpt-only republication (body_html never copied), `hosted_on` platform → venue slot, artists/labels → first-class EntityRefs (`az:` ids — real affinity edges), **no `mentions`-as-HL** (archive items carry no hp; 2010-era dates decay them to the rim). Original CDNs are dead → every image is rewritten through Wayback's `im_` modifier at the post's capture timestamp and **verified** (sequential, 500ms pauses kept per MANUAL §3, 429 backoff, results cached in archiveImageCache.json so re-runs are offline). **Yield: 44/85 posts** (39 Wayback art + 5 YouTube thumbs); 41 excluded — no recoverable image.
- **Map integration** — /mapa merges the archive seed (map-only; never the home feed). New `ContentSource` member `'archive:wayback'` (write-boundary guarded: contentItemToRow/seed.ts null it — the DB enum gains the value only when the real Archivo Vivo schema lands). Cells: aged-paper rim `#9C8F7F` + `//ARCHIVO` chip + `blog · año` credit line. Overlay: type `articulo` → ArticuloOverlay renders excerpt + CONTEXTO links (Wayback + blog).
- **Image-only terrain** — rule 1 amended: items without imagery never enter the map (20 contemporary seed items excluded; they remain on section pages/search).
- **HL variation** — 14 dev-seed items stamped with fresh varied hp (45-140): terrain now 219×1 / 10×3 / 4×7 (was 204/4/1 pre-stamps). Beta posture, dev seed only, no prod meaning.
- **Verified**: 42/42 tests (new archive.test.ts: image-only, az-prefix uniqueness, visible-credit contract incl. no-full-body, entity-ref wellformedness, no-hp, deterministic order) · `tsc` clean · lint clean · prod build clean · live: terrain shows contemporary core with varied polyhexes + archive band at the periphery (mean radial 12.7 vs 11.3 HEX_R), Wayback images paint slowly but paint.
- **Gotcha repeated so it sticks**: ran `next build` while dev served → chunk-map corruption again (unstyled fragments). Restart dev after builds.
- **Next (named, not started)**: the canonical Archivo Vivo schema goes in THIS wiki per `_schema.md`/MANUAL §5 — one shared `entities` table with contemporary Gradiente, role-as-value `post_entities`, era visibility policy, alias merge flow. The map pilot changes none of that; it's presentation over the file-side seed.

---

## 2026-08-18 · INGEST · `/mapa` pass 7 — relevance belt, affine-partner carousel, individual listing detail · [[Mapa Placement Rules]]

Iker's review: (1) listing cells opened the ENTIRE partner marketplace (grid + detail stacked) instead of the individual listing; (2) during focus the exterior should REASSEMBLE by affinity — related content and partners gathering around the focused cluster, affine partners navigable "carousel fashion".

- **Individual listing detail** — the map now mounts [[MarketplaceListingDetail]] DIRECTLY (it's self-contained: `{listing, partner, index, onClose}`), skipping [[MarketplaceOverlay]]'s grid entirely. Listing hex → that listing's canonical detail → one ESC → back on the focused map with `?focus=` intact. Deep link `/mapa?focus=X&partner=X&listing=Y` unchanged. (The hexes were always individual listings; the OPEN was the wholesale part.)
- **Relevance belt** ([focus.ts](../lib/mapa/focus.ts)) — focus now builds a three-ring gradient: core (members, type arcs, nucleus) → **belt**: the ≤18 most-affine non-members (pairwise affinity to members ≥ 2 — the time-term alone can't qualify) reassemble just outside the one-ring buffer, translation-only, each preserving its **original bearing from the nucleus** so it slides inward rather than teleporting; → far terrain dims in place. Belt items keep full color (the dim set is now non-member ∧ non-belt); static colliders displace outward past core+belt. Rule 10 of [[Mapa Placement Rules]] rewritten accordingly.
- **Affine-partner carousel** — `rankRelatedPartners(layout, clusters, focused)`: other clustered identities ranked by mean of top-3 cross-cluster pairwise affinities (pure, no behavior). The obi gains a `//PARTNERS AFINES` row with ‹ › stepping — focusing the next-most-affine partner glides the camera across the shared terrain into that partner's reflow. (Dev seed has one cluster → row hidden; prod's 3 clusters light it up.)
- **Tests 37/37** (new: belt gathers the venue/genre-sharing exterior adjacent to the cluster + outside the buffer; unrelated exterior stays out; carousel ranks the genre-sharing partner first) · `tsc` · lint · prod build clean (`/mapa` 13.3 kB).
- **Verified live** (dev seed, desktop-width pane): focus shows core + 3 MERCADO nodes + full-color belt (Surgeon/FASCiNOMA/hard-techno pieces ringing the cluster) over dimmed far terrain; listing click → individual detail only (no marketplace chrome) → ESC → map intact. Pane gotcha for the log: a hidden pane tab reports `innerWidth 0` → the map boots with a zero viewport and virtualization collapses until the ResizeObserver re-measures on reveal — self-heals, but explains "1 cell" probes on hidden tabs.

---

## 2026-08-18 · INGEST · `/mapa` pass 6 — REAL DATA: prod validation, full partner roster, layout cache, migration 0046 (PENDING) · [[Spatial Identity Canvas]]

Iker: "now let's use real data — all partners, all info in the actual site." Grounded the map in the actual prod DB (read-only service-key audit + engine run; temp scripts deleted after):

- **Prod reality** (2026-08-18): 459 published rows — 326 eventos · 78 partners (ALL with logos; 29 marketplace_enabled; 1 real marketplace_listings row) · ~55 editorial-family; 160 seed + 299 real. Attribution: 6 items across THREE partners — `pa-club-japan-ppur` (3), `pa-fhauna-wkwu` (2), `pa-jungle-empire` (1). Content coverage: images 339/381 (~20 domains incl. images.ra.co ×103, supabase storage ×179, one giphy GIF), venue 85%, artists 61%, genres only 48%, created_by 43 — so real-data affinity leans venue/artists/time more than genres. Only 4 of the 11 CJ-related dev-seed rows exist in prod (ev-japan-industrial/rat-pack/hardtechno-may, ar-002).
- **Engine validated on the REAL dataset**: 381 terrain items place in **122ms**, 427 cells, sizes 366/11/4 (real fresh content earns polyhexes), zero integrity violations; all three real clusters reflow (club-japan 8 moves · fhauna 17 · jungle-empire 8, ≤2ms each).
- **Layout memo** — [app/mapa/page.tsx](../app/mapa/page.tsx) caches `{layout, clusters}` per (10-min now-bucket + rolling hash of id/hp/hpLastUpdatedAt/partner_id), cap 3 — amortizes the ~120ms across a server instance's force-dynamic requests.
- **Partner selector** — the chip row died at 78 partners. New `◎ PARTNERS` panel in [MapaCanvas](../components/mapa/MapaCanvas.tsx): `//CON TERRENO` (focusable identities with logo + publication count) above `//SIN CONTENIDO EN EL MAPA` (the rest of the roster, alphabetical, inert — honest index, no fake affordances). Page now passes ALL partner rows; itemsCache records them; terrain pointerdown dismisses the panel.
- **Migration `0046_mapa_partner_attribution.sql`** (PENDING — apply via SQL editor, never `db push`): idempotent stamps of the 4 prod-present CJ seed rows onto `pa-club-japan-ppur` (3 publisher nights + ar-002 editorial attachment) → real CJ cluster grows 3 → 7. Marketplace listings deliberately NOT seeded to prod (fake buyable products on the live site — no; the listing-node demo stays dev-seed).
- **Verified**: 34/34 tests · `tsc` clean · lint clean · prod build clean (`/mapa` 11.3 kB) · live (dev seed): selector lists 1 clustered + 4 inert partners, panel-select → focus reflow + obi + nucleus, terrain click dismisses. Real-data engine behavior verified via the service-key script (above); the authed prod UI walk needs Iker's login.
- **Open**: with 326/381 eventos the real terrain is event-dominated (honest, but evaluate whether the map wants the home-style flood caps); genre backfill would strengthen affinity; FHAUNA/Jungle Empire obis will be sparse (no excerpt/location fields populated).

---

## 2026-08-18 · INGEST · `/mapa` pass 5 — marketplace listing nodes + mindshare placement rules (identity bearings) · [[Mapa Placement Rules]]

Two Iker directives: marketplace listings in the focus cluster, and CONCISE placement rules — "mindshare geographical sections within our honeycomb."

- **Marketplace nodes** — listings join the focus cluster as single-hex MERCADO nodes ([focus.ts](../lib/mapa/focus.ts) places them as an arc after the content types; [MapaListingCell](../components/mapa/MapaListingCell.tsx) renders photo + `//MERCADO` chip + `$precio MXN · estado` in the spec's warm-sand rim `#D6B37A` — marketplace has no ContentType so `categoryColor` can't supply it; recorded for the palette pass). Clicking opens the CANONICAL `?partner=&listing=` [[MarketplaceOverlay]]/[[MarketplaceListingDetail]] mounted OVER the map (it's a self-contained URL-driven component — the listing detail opens without leaving the surface; deep link `/mapa?focus=X&partner=X&listing=Y` restores it). Seed: `pa-club-japan` is now marketplace-enabled with 3 listings (the mockup's East Asia Electronics Network Vol. 3 LP + cassette + camiseta) and a real `marketplaceLocation` ("Monterrey 56, Roma Norte, CDMX") — which also lights up the obi's UBICACIÓN. Global-terrain listing nodes still deferred.
- **Mindshare geography** — new [[Mapa Placement Rules]] note: the TEN rules (every cell is content · one item one slab · affinity chooses neighbors · **identities own directions, not territories** · regions emerge with bridges · types intersperse globally · center is now, rim is memory · slabs are separate pieces · stable and shared · focus reorganizes). The new mechanism is **identity bearings** ([layout.ts](../lib/mapa/layout.ts) `identityBearing` — djb2 + murmur avalanche → fixed angle per identity id; `W_ANCHOR` pull on attributed items toward an anchor at that bearing): a partner's section always grows in the same compass direction, giving the map learnable macro-geography without drawn territories. Gotcha caught by the new tests: raw djb2-mod gives near-identical bearings for ids differing in the last char — the avalanche finalizer is load-bearing.
- **Stability micro-fix** — [app/mapa/page.tsx](../app/mapa/page.tsx) quantizes the layout `now` to 10-minute buckets so soft refreshes (e.g. the marketplace overlay's `router.replace`) can't flip an HP near-tie and reshuffle terrain (placement-rules rule 9).
- **Tests 34/34** (new: listing nodes attach without overlap + stay contiguous; bearing determinism; opposing bearings → clusters grow in different directions; direction stable when unrelated content is added) · `tsc` clean · lint clean · prod build clean (`/mapa` 10.9 kB).
- **Verified live**: focus shows the 11-item cluster + 3 MERCADO nodes + nucleus; listing click → marketplace detail over the map with URL params; double-ESC unwinds detail → grid → map with `?focus=` intact; desktop obi now shows the real address.
- **Open** (recorded in the rules note): cross-snapshot hysteresis beyond the `now` bucket, genre-community bearings (or is that category districts by the back door?), global marketplace nodes, tunables editorial pass.

---

## 2026-08-18 · INGEST · `/mapa` pass 4 — partner-focus REFLOW: self-contained type-ordered cluster + identity nucleus · [[Spatial Identity Canvas]]

Iker's ask: zooming into a partner should REARRANGE the honeycomb into a self-contained section neatly arranged by content type — and Club Japan's 5 attributed items were too few to show it. Both halves built:

- **Reflow engine** — [lib/mapa/focus.ts](../lib/mapa/focus.ts) `computeFocusArrangement(layout, cluster)`, pure + deterministic: members gather around an **identity nucleus hex** placed at the global cluster's centroid (continuity — the reflow happens where the cluster already lived); placement walks `FOCUS_TYPE_ORDER` with same-type-contact scoring, so types land as tidy adjacent arcs within ONE contiguous cluster (a neat archive, not category districts); non-member terrain colliding with the expanded cluster + one-ring buffer glides outward along the nearest hex direction. **Translation-only** — every item keeps its exact polyhex shape/orientation, so cells stay recognizable through the transition (spec § zoom continuity). Returns per-item px deltas (cells animate via a 700ms transform transition, reduced-motion instant), the reflowed perimeter ring (outset via negative `insetLoop` gap), the identity hex geometry, and a derived layout so arrow-key traversal follows the REFLOWED geometry while focused.
- **Identity nucleus** — the mockup's central CLUB JAPAN hex: dark hex with logo + wordmark + kind/subtitle, rendered ONLY in focus state, links to the canonical `/p/[slug]` dossier. DEVIATION from the spec's "partner identity never consumes terrain cells" — the mockups clearly show it and it anchors the reflow; treated as the "selected-state banner" materialized during focus. Flag for datavismo alongside the type-chip and type-arc deviations (spec says "content types remain interspersed" in focus; Iker's direction + mockup reading = neat type arcs).
- **Attribution enrichment** — Club Japan now has **11 seed members across 7 types**: the 5 publisher stamps from pass 1 + 6 new editorial attachments (precedence-5 "explicitly about": rv-003, rv-006, ed-001, no-002, li-hard-techno-cdmx-2026, ar-002 — each genuinely about the venue). Chosen over inventing a synthetic partner: keeps one demo identity and mirrors how prod attribution matures. Marketplace listing nodes remain a later phase.
- **Camera** targets the reflowed bbox (reflow + zoom travel as one gesture); virtualization + `ensureItemVisible` use delta-adjusted bboxes; the arrangement is cached per partner slug (stable delta identities for cell memoization).
- **Tests** — new [tests/mapa/focus.test.ts](../tests/mapa/focus.test.ts): pixelToHex roundtrip, arrangement determinism, identity+members contiguity, translation-only shape preservation, no doubly-owned cells in the derived layout, non-members fully outside cluster+buffer, same-type adjacency (every member with a type peer touches one). **30/30 green** · `tsc` clean · lint clean · prod build clean (`/mapa` 10.6 kB).
- **Verified live**: chip → 11 cells glide into the nucleus-centered cluster (types in arcs, full color) while surroundings dim beyond the ring; obi + deep link + Back intact; ZOOM GLOBAL reverses every delta to zero and unmounts nucleus/ring.

---

## 2026-08-18 · INGEST · `/mapa` pass 3 — glass layer REMOVED, seamless slabs, `<img>` cells (GIFs play), perf rebuild · [[Spatial Identity Canvas]]

Iker's pass-2 review: interior unit edges still visible inside multi-hex items, the glass layer distorted everything, GIF content stopped animating, and the page got LAGGIER. All four traced to the same root and fixed together:

- **Glass layer removed** (GlassLayer.tsx deleted, `?glass=0` machinery gone). It was both the distortion complaint AND the new lag: a continuous rAF loop drawing 209 shader quads (~35 SDF evals/fragment) every frame on top of the DOM. The design (hex-SDF refraction, curvature lighting) survives in the pass-2 log entry if a WebGPU/TSL attempt ever revives it; verdict for now — the DOM look is the product.
- **Seamless polyhexes** — interior seams cut entirely: `PlacedItem.seams` + `seamsPath` deleted; a multi-unit item is one unbroken slab (spec's "fine graphite internal seams MAY remain visible" resolved to NO). The smin-crease variant died with the shader.
- **Cells are now HTML `<img>` + CSS `clip-path: path(outline)`** instead of SVG `<image>`. Two birds: (1) **GIFs animate again** — SVG `<image>` only ever paints frame one in every browser, which is why prod GIF flyers froze; (2) **zoom stops re-rastering** — HTML images are GPU-composited bitmaps that scale at composite time, while SVG content re-rasters per scale change. The remaining per-cell SVG is a stroke-only rim path (tiny). Legibility shade + hover lift are gradient/tint overlay divs inside the same clip (hover is an overlay tint now, not a brightness filter).
- **Perf levers**: virtualization margin tightened 0.85 → 0.55 (113 mounted cells vs 173 at global zoom on the seed), text-shadow dropped from titles (the bottom gradient carries contrast), `[contain:layout_style]` + plane `will-change` + `.mapa-zooming` transition-suspension retained from pass 2.
- **Verified**: `tsc` clean · 23/23 tests · lint clean · prod build clean (`/mapa` 8.1 kB, three.js chunk gone) · live: rosette + trihexes render as unbroken slabs with grout, overlay opens from the rosette, focus dim (media-stack variant) + obi intact, deep link works. GIF playback is the guaranteed `<img>` behavior but should be eyeballed against prod data (dev seed has no GIFs). REAL frame-feel still needs Iker's machine — the dev browser pane throttles rAF to ~0, so fps can't be measured in-session.

---

## 2026-08-18 · INGEST · `/mapa` pass 2 — WebGL glass slabs + slab grout + focus dim + zoom perf · [[Spatial Identity Canvas]]

Iker's review of the first slice: (1) no clear indication of partner content on focus, (2) the reference-video glass interaction wasn't implemented, (3) cells need padding between slabs, (4) zoom was extremely laggy (text-raster bound). All four addressed:

- **Glass layer** — [GlassLayer.tsx](../components/mapa/GlassLayer.tsx): three.js (already in the stack) WebGL pass painting the slab BODIES under the DOM text/interaction layer. GLSL per the reference thread, hex-adapted: IQ hex SDF unioned via smooth-min (welds a polyhex into one slab, soft undulations at unit seams), distance-driven circular-shoulder bevel → normals rebuilt from SDF gradient + **sphere-curvature tilt from viewport offset** (highlights travel across slabs as the grid pans — the endless-curved-grid feel), refraction UV offset through the slab with 3 IOR samples for chromatic aberration, fresnel over a procedural env + tight specular, category rim in-shader. Textures LRU-capped at 90 (full-res flyers would blow GPU memory); CORS/404 textures hand the cell back to its SVG (`keepSvg`); no WebGL or `?glass=0` → full SVG fallback. Render-on-demand (camera-change dirty check), ortho frustum synced to the pan/zoom camera each rAF. Mac/Metal NaN guards per `reference_mac_metal_shader_nan`.
- **Slab grout** — `HEX_GAP = 5`: every polyhex outline insets (SDF `d + gap` in-shader; true bisector polygon offset in [polyhex.ts](../lib/mapa/polyhex.ts) `insetLoop` for the SVG path + rim + clip), seams trimmed to match. Items now read as separate slabs with dark grout — the mockup texture. 2 new unit tests (apothem-exact single-hex inset, trihex inset containment); 23/23 green.
- **Focus clarity** — partner focus now dims + desaturates every non-attributed cell (shader `uDim` / CSS fallback `.mapa-cell--dim`) while attributed cells keep full color with a boosted rim + inner glow (`uEmph`). Combined with the perimeter + obi, the partner's content is unmistakable. Type labels became mockup-style boxed chips (category-colored border/text on dark) — NOTE: deviates from the spec's strict "type color only on the rim" in favor of the approved mockups; flag for datavismo with the rim-palette question.
- **Zoom perf** — root cause: scaling the CSS plane re-rasterized every SVG image + text-shadowed label per frame (the "threshold where lag disappears" = the far-band text cutoff). Fixes: imagery moved to the GPU (SVG hidden in glass mode), `[will-change:transform]` on the plane, `[contain:layout_style]` per cell, and a `.mapa-zooming` class that suspends text transitions during scale changes so band flips can't start a mass opacity transition mid-gesture. Also fixed +/− buttons compounding from the in-flight zoom target instead of the mid-animation value. CAVEAT: the in-app browser pane throttles rAF (~0 idle fps), so real interactive frame rates need verification on Iker's machine.
- **Gotcha (pane, not code)** — the pane served a stale/corrupt cached chunk that threw `SyntaxError: Invalid or unexpected token` and silently killed hydration across loads; `fetch(src, {cache:'reload'})` + reload cleared it. Recorded in `browser-pane-quirks` memory.
- **Verified**: `tsc` clean · 23/23 tests · lint clean · prod build clean (`/mapa` 9.6 kB + lazy three chunk) · live: glass bevels/refraction/chroma at near zoom, grout in both modes, focus dim (48 dimmed / 5 emphasized), obi, deep link, `?glass=0`.

---

## 2026-08-18 · INGEST · Spatial Identity Canvas — `/mapa` vertical slice BUILT (experimental route, uncommitted) · [[Spatial Identity Canvas]]

First working prototype of the honeycomb canvas, on a new experimental route — production home/partner/user surfaces untouched. **Phase 0/1/2 slice + one partner-focus demo (Club Japan).**

- **Engine** — `lib/mapa/` pure-function layout modules (no React): `hex.ts` (flat-top axial coords, neighbor/edge geometry), `polyhex.ts` (1/3/7 template vocabulary, connectivity, exterior-edge detection, SVG outline/seam builders), `affinity.ts` (deterministic metadata affinity: partnerId · creator · entities · artists · venue/city · genres exact+root-rollup · taxonomy-filtered tags · date proximity; `ra` provenance tag excluded; legacy↔leaf genre ids meet at the root), `layout.ts` (greedy frontier placement w/ repeated-type penalty + compactness + radial bias; HL→1/3/7 reusing [[curation]] `score()` thresholds 1.0/0.5, dominant cap 4, plain events never dominant — mirrors the home mosaic's curator-intent rule; `partnerClusters` = explicit `partnerId` attribution ONLY; `neighborItemId` for keyboard nav). Layout computed ONCE server-side per request (~50ms for 209 items) and hydrated as props — deterministic server/client by construction.
- **Route** — [app/mapa/page.tsx](../app/mapa/page.tsx) (force-dynamic, `getItems()` like home; MOCK_ITEMS dev-seed fallback when RLS returns zero rows for anon dev). [MapaCanvas](../components/mapa/MapaCanvas.tsx): imperative camera (pan/wheel/pinch/momentum off the React render path), semantic zoom bands (`data-band` far/mid/near gate cell text via CSS in globals.css), viewport virtualization, roving-tabindex six-neighbor arrow nav (↑↓←→, ⌥ for the opposite diagonals), `?focus=` pushState + popstate (Back restores camera), `recordItems()` bridge so cells + `?item=` deep links resolve through [[OverlayRouter]]. [MapaCell](../components/mapa/MapaCell.tsx): SVG polyhex — full-color `<image>` clipped to the union outline, graphite interior seams, type-colored rim on the EXTERIOR boundary only (existing `categoryColor`), glass sheen per the reference video, semantic `role="button"` + aria-label. [PartnerObi](../components/mapa/PartnerObi.tsx): Japanese-vinyl-obi identity strip (left band ≥lg, bottom sheet below), all real fields — KIND, wordmark + クラブ・ジャパン accent, UBICACIÓN (subtitle fallback), HORARIO SEGÚN EVENTO template line for venues, CONTACTO derived from partnerUrl (`@JAPAN_CDMX`), N publicaciones, `/p/[slug]` canonical link, − ZOOM GLOBAL.
- **Chrome/gating** — [[Navigation|ChromeFrame]] nulls on `/mapa` (full-viewport, own minimal chrome); middleware gives `/mapa` the dev-only anon allowance (prod stays invite-gated like every content route — no prod middleware change).
- **Seed attribution** — stamped `partnerId: "pa-club-japan"` on the 5 mock items Club Japan itself publishes (4 venue nights + the Mixcloud Best-Of), mirroring prod's explicit `items.partner_id` stamps on `pa-club-japan-ppur`. Items ABOUT Club Japan (ed-001, rv-003, no-002…) deliberately NOT attributed — they cluster nearby via affinity instead, which is the demo of spec open question #10.
- **Tests** — `npm run test:mapa` (tsx --test, no new deps): 21 tests over deterministic placement (byte-identical, input-order-free), affinity properties, polyhex connectivity, exterior-edge counts/loops. All pass; `tsc` clean; lint clean; prod build clean (`/mapa` 8 kB ƒ).
- **Verified live on :3003** — global terrain → chip → animated focus into the Club Japan cluster (attributed events contiguous, related editorial as recognizable neighbors), obi sheet, cell click → real [[EventoOverlay]], close → map state intact, `?focus=` deep link, Back restore, wheel/drag pan (+ click suppression), arrow-key neighbor traversal.
- **Gotcha caught** — `setPointerCapture` on pointerdown retargets the ensuing `click` to the capturing container and silently kills cell opens; capture must start only after the drag-movement threshold.
- Deviations, assumptions, and open product decisions recorded in [[Spatial Identity Canvas]] § Prototype findings.

---

## 2026-08-18 · INGEST · Spatial identity canvas concept — global honeycomb → partner/user focus · [[Spatial Identity Canvas]]

Documented the design exploration for replacing conventional partner/user profile pages with one continuous, zoomable honeycomb canvas. The draft specifies global → approach → identity-focus semantic zoom, content-only occupied cells, full-color imagery with type-colored rims, deterministic metadata-affinity placement, HL/HP-driven 1/3/7-cell polyhex weight, partner/user identity strips, explicit partner attribution, route/SEO preservation, owner/dashboard boundaries, accessibility, responsive behavior, data requirements, implementation phases, acceptance criteria, and open questions. The proposal preserves [[Size and Position as Only Signals]] and [[No Algorithm]] but would require explicit revisions to [[Partners Isolation]], [[Contained Single Surface]], [[Partner Page]], and [[ContentGrid]] before implementation.

---

## 2026-08-10 · INGEST · Public waitlist — /espera page + admin //ESPERA tab (BUILT, uncommitted; migration 0045 pending)

The viral-campaign waitlist: visitors without an invite code can now leave alias + email and get queued; admins convert entries into REAL invite codes that funnel into the existing `/welcome?codigo=` 3D signup pipeline. **The waitlist never touches `auth.users`** — it's purely a feeder for the shipped invite system, so there is no second signup path to maintain.

- **`/espera`** ([app/espera/page.tsx](../app/espera/page.tsx)) — full-bleed terminal cockpit recreating the reference mockup with /welcome's exact tokens (bg-base scanlines, sys-orange, corner-bracket Panels, mono 10px labels): left telemetry (brand, NODO ACTIVO, RED DE NODOS, **ESTADÍSTICAS DE INVITACIÓN — real numbers** from `GET /api/waitlist`, FRECUENCIA + waveform), procedural **ASCII transmission eye** centerpiece (104×56 `<pre>` renderer à la VinylAscii: mandala spokes, almond/iris/pupil, beam + pedestal, blink every ~7s, reduced-motion static), MÓDULO DE ESPERA form (alias ≤20 + live counter, email + format tag, ciudad/origen selects, honeypot), right column (INTENSIDAD 87%±1, DESCIFRANDO MENSAJE typewriter, NOTA DEL OPERADOR), 4-col footer. Success state shows **queue position** (`#00N`) + persists to localStorage (`gradiente:espera`) so returning visitors see their spot; "registrar otro correo" resets. Phones: asides + eye hidden, form-first, 16px inputs, no h-overflow (verified 375px).
- **Plumbing** — [lib/waitlist.ts](../lib/waitlist.ts) shared city/source allowlists (form + route import the same constants). [`/api/waitlist`](../app/api/waitlist/route.ts): POST = service-role insert with honeypot fake-success, allowlisted selects, lowercased-unique email, **idempotent duplicate → returns existing position**; GET = real counts (60s edge cache). Missing-table (42P01/PGRST205) → friendly 503, so the page survives a deploy-before-migration window. Middleware allowlists `/espera` for anon + bounces authed users; [[Navigation|ChromeFrame]] now nulls on `/espera` too (nav/player/slider no longer mount behind the cockpit). `/welcome` got a quieter third CTA: `¿SIN CÓDIGO? >> UNIRME A LA LISTA DE ESPERA <<`.
- **Admin //ESPERA tab** ([AdminWaitlist.tsx](../components/admin/AdminWaitlist.tsx) + [`/api/admin/waitlist`](../app/api/admin/waitlist/route.ts)) — queue-ordered table (position/alias/email/ciudad/origen/fecha/estado) with stats chips + filter. Per-row **▶ GENERAR CÓDIGO** mints a real invite code (folio continuation, `card_name` = alias, 30-day expiry, idempotent — an unexpired existing code is reused) and marks the row INVITADO; **COPIAR ENLACE** hands the `/welcome?codigo=` deep link for manual delivery (DM/mail — personal-beta posture; automated email is a later slice). **REGISTRADO is DERIVED** from the joined `invite_codes.used_at` (embed over the `invite_code` FK) — no second write path. Two-click BORRAR for spam/ARCO removals.
- **Schema — migration `0045_waitlist.sql`** ([supabase/migrations/0045_waitlist.sql](../supabase/migrations/0045_waitlist.sql)): `waitlist_signups` (email unique lowercased, alias, city, source, status pending|invited, invite_code FK → invite_codes.code, invited_at) + `waitlist_signups_admin_all` RLS via `private.auth_is_admin()` (public writes go through the service-role route only). **PENDING: apply via the SQL editor** (MCP is read-only; confirmed live — `apply_migration` refused). Never `db push`. Until applied: POST 503s friendly, stats read 0, admin tab reads empty — all graceful, so apply-then-push or push-then-apply are both safe windows.
- **Verified**: `tsc` clean, prod `next build` clean (`/espera` 7.82 kB static), live walkthrough on :3003 — anon reaches `/espera`, validation errors in Spanish, POST → 503 friendly (table absent), stats zeros, /welcome CTA navigates, mobile 375px form-first. Full-flow (real insert → position) needs 0045 applied.
- **Design calls**: stats panel is REAL data (0s at launch read as "nobody's in yet" exclusivity — honest per the redesign doctrine); atmosphere panels (logs, nodos, actividad) follow the /welcome fake-telemetry precedent; mockup's "VERIFICADO" tag became a live FORMATO OK check (no decorative chrome). Email strategy decided: **manual link delivery for wave 1**, Resend (~3k/mo free, SPF/DKIM on gradiente.org) when volume demands; `auth.admin.inviteUserByEmail` rejected (bypasses the invite-code trigger + T&C/username/3D-card flow).
...
- **Genres from RA, not inference.** RA's GraphQL exposes `event.genres` (~82–85% of CDMX events tagged) — the scrape just wasn't requesting the field. The agentic scrape now selects `genres { name }` and passes the raw RA display names; the route maps them → Gradiente taxonomy ids via a new [lib/scrapedGenres.ts](../lib/scrapedGenres.ts) (`mapScrapedGenres`, ~40 RA-name→id entries incl. IDM / Minimal Techno / Progressive House / Post-Punk / Hardcore / Latin Bass). Unmapped names drop; values already valid ids pass through.
- **Deterministic seed vibe.** `seedVibeFromGenreIds` derives a provisional `[min,max]` band from the mapped genres' stereotype anchors (self-contained `SEED_VIBE_BY_ID`, mirrors the deprecated `GENRE_VIBE`): one anchored genre → ±1 band, multiple span their anchors. Explicitly a stereotype seed, NOT a verdict — honors [[Vibe Philosophy]] idea 2 ("genre is a lie").
- **Migration 0036** ([supabase/migrations/0036_scraped_event_vibe_seed.sql](../supabase/migrations/0036_scraped_event_vibe_seed.sql)): `ingest_scraped_event` gains `p_vibe_min`/`p_vibe_max` (default 5), via DROP+CREATE (one overload, no ambiguity). Seeds on INSERT; **on conflict re-seeds vibe ONLY while the row is still the untouched default 5/5** — an editor-graded band, or the crowd [[Vibe Checks]] median (which overrides at read time), is preserved exactly. Backward-compatible (the old 17-arg call resolves via defaults), so the migration is safe to — and MUST — apply BEFORE the route deploys (the new route calls the 19-arg signature; an old route would write raw names + skip the seed).
- **Backfill.** Re-scraped July (50 CDMX events) through the new route → **41 genre-tagged + vibe-seeded, 9 untagged-by-RA stay 5/5, 0 raw-name leaks**. Vibe spread now 2→10 (house/techno-heavy middle: `3-5` ×15, `3-7` ×9, `5-7` ×6). Confirmed the new deploy was live first via a canary (`Latin Bass`→`latin-bass`, vibe 6-8 — a `0a69105`-only mapping) before backfilling. Reads stay global (No-Algorithm-safe). Runbook in `ra-scrape-flow` memory; supersedes [[Scraper Pipeline]] decision #2.

---

## 2026-06-25 · FIX · /welcome código form unclickable on short (laptop) viewports

Invite-code input + ▶ ACTIVAR on [/welcome](../app/welcome/page.tsx) were reachable only by keyboard TAB/ENTER, never mouse — but ONLY on laptop/MacBook-height screens (fine on tall desktop monitors + iPad). Cause: the decorative ASCII-vinyl `<pre>` (`aria-hidden`) carries `filter: drop-shadow`, which makes it a **stacking context** — so it paints (and hit-tests) ABOVE later non-positioned siblings despite coming earlier in the DOM. On short viewports its fixed 64-row grid overflows its `flex-1` box downward over the form, and with no `pointer-events`, it swallowed the clicks. Keyboard still worked because focus order ignores stacking. Fix (`c8a5a95`, 1 line): `pointer-events-none` on the vinyl `<pre>`. Verified in the dev preview at 1280×720 via `elementFromPoint` over the input/button centers — returned the vinyl `<pre>` before, the input/button after. Same trap class as the [[RegistroCard]] T&C modal needing `createPortal` to escape the 3D overlay's `pointer-events:none` stacking — decorative/overlay layers on /welcome must be `pointer-events:none`.

---

## 2026-06-25 · INGEST · Home grid — events interleave by urgency + global mosaic cap (SHIPPED to main)

Merged Johan's PR branch `fix/home-events-cap` + a cap fix to `main` (`3f25966`, live on gradiente.org). Two coupled commits: **673b524** (Johan) flipped event placement in [[curation]] from "block strictly BELOW all content" (`prominence = -1 + proximity`) to **interleave by time-urgency on the same 0–1.2 scale as content** (`eventDateUrgency + 0.3·score`, where `eventDateUrgency = 1.2·e^(−daysUntil/10)` → tonight ≈1.2, a week out ≈0.6) — so an imminent party rises into the hot band and a far-off one settles among older posts. **34219fc** (me) added the missing **total** bound: the PR only had a per-DAY cap (`EVENTS_PER_DAY=3`), which never bounds the sum — 19 upcoming days × 3 surfaced **42 events into a 39-content grid (~52% events)**. Added `MAX_MOSAIC_EVENTS = 8` in [app/page.tsx](../app/page.tsx): nearest-date scraped events fill up to 8, the rest stay rail-only ([[EventosRail]] already carries the full firehose). Curator picks (`editorial`/`elevated`) bypass BOTH caps (intent; 0 today). Result verified by simulating the exact algo against prod: **42 → 8** (3 from Jun-26, 3 from Jun-27, 2 from Jun-28 = the most urgent), grid now ~17% events. Size was already guarded (plain events forced to 1×1 md in `rankItems`). **Why a fixed cap, not a ratio (Iker's beta-growth question):** an absolute ceiling self-corrects as community uploads grow the grid — event share shrinks on its own (8/39 ≈17% → 8/300 ≈3%) with no re-tuning; a 40%-ratio would do the *opposite* (more scraper noise the busier the site). tsc clean; no local `next build` (Iker's dev server was running — Vercel builds fresh on deploy). Knobs: `EVENTS_PER_DAY` (drop to 2 for a wider calendar spread) + `MAX_MOSAIC_EVENTS`, both at the top of the event-selection block in [app/page.tsx](../app/page.tsx). See `project_home_event_placement` memory.
