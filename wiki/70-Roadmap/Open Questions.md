...
- **Username could previously contain `@` and other URL-unsafe characters** — closed-but-deferred 2026-05-22. The signup route is now gated against `^[a-z0-9_-]{3,30}$` (commit `e8fbade`), so no new account can reach the bad state. Five existing accounts still have email-shaped or dotted usernames (`datavismo@gmail.com`, `ED.`, `pro.tone`, `yorkajorge@gmail.com`, `adios.quimera`); their card chips and `/u/<name>` URLs render the literal value, which is ugly but functional. **Decision: do NOT mass-rename existing handles** — that's a user-facing identity change that needs each affected user's buy-in, not a one-sided cleanup. Revisit only if/when an affected user asks for their handle to change, or if the literal-value URL ever breaks something concrete.
...
- ~~**`ArticleCard` `TYPE_LABEL` map**~~ — **fixed 2026-04-22.** Added `opinion` and `franja` entries to satisfy `Record<ContentType, string>`. (Component since removed 2026-06-23 — dead code deleted.)
...
- **`body` field on `ContentItem`** — RESOLVED DIFFERENTLY. Longform landed as structured `article_body` + `footnotes` ([[ArticuloOverlay]], [[ListicleOverlay]]), not a markdown `body` string; `externalUrl` for Substack-hosted pieces was never built. [[ReaderOverlay]] still renders `bodyPreview` only.
...
- ~~**Articulo dashboard form**~~ — **shipped 2026-04-25.** Self-contained ArticuloForm with full block editor (all 10 kinds) + footnotes editor. Closes dashboard type coverage at 8 of 9 (only `franja` excluded).
...
- **Overlay for `franja` type** — currently franjas are not clickable (they live in [[FranjasRail]] only, not clickable-into). Is that the final stance, or should franja cards open an overlay with their promo info / external link?
...
- Franja rail — add mobile layout or leave as desktop-only?
...
- **Espectro → Gradiente content migration** — mix titles (`ESPECTRO MIX ###`), bylines (`Redacción Espectro`), slugs, franja field references. Coordinate with the scraper cutover.
