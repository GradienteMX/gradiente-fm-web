...
- **Username can contain `@` and other URL-unsafe characters** — surfaced 2026-05-22 during Bundle A verification of [[Public Profile Route]]. One existing user has `users.username = 'datavismo@gmail.com'`, which means their card chip renders as `@datavismo@gmail.com` and their public profile URL is `/u/datavismo@gmail.com`. The signup route accepted an email-shaped string as a username at some point. **Two fixes needed:** (1) one-off `UPDATE users SET username = '<chosen-handle>'` for the affected row — confirm the handle with the user first; (2) harden `app/api/auth/signup/route.ts` to validate against a slug-like regex (e.g. `^[a-z0-9_-]{2,30}$`) so an email or anything with `@` / `.` / whitespace can't slip in. The username is now load-bearing for `/u/[username]`, the CreatorChip on cards, and the future profile/HP surfaces, so this is no longer "cosmetic." A spawned-task chip exists for this.
