...
type: roadmap
status: draft
tags: [spatial-ui, honeycomb, franjas, users, identity, affinity, hl, navigation]
updated: 2026-08-18
---

# Spatial Identity Canvas

> Draft specification for replacing conventional franja and user profile pages with one continuous, zoomable honeycomb canvas whose geometry is organized by content affinity and HL.
...
- The current [[Franja Page]] at `/f/[slug]` remains live and authoritative until this proposal is implemented.
- The current `/u/[username]` profile remains live.
- The franja direction is comparatively well defined after several visual concept passes.
...
3. **"About the franja" ≠ attributed.** Focus eligibility is `items.franjaId` only (precedence rules 1–4). Editorial ABOUT Club Japan is not attributed in the seed; affinity places it adjacent to the cluster instead. This reads as the right answer to open question 10 — attribution stays strict, discovery happens through geography.
4. **Seed attribution added.** 5 mock items Club Japan publishes got `franjaId: "pa-club-japan"` stamps (mirrors prod's 3 attributed events on `pa-club-japan-ppur`). Attribution is hand-stamped editorial data, never inferred at runtime.
...
- **Franja identity data is thinner than the obi wants.** No address, schedule, or contact fields exist on franja rows: location falls back to `subtitle` ("Venue · Roma Norte"), schedule is the honest template line `HORARIO SEGÚN EVENTO` for venues, contact is derived from `franjaUrl` (`@JAPAN_CDMX` parsed from the Instagram URL). A real obi needs either franja-row columns (address/schedule/contact) or a structured link to the venue's `entities` row (which has `address` since 0039).
- **`getItemsByFranja()` is lean** (no entities/creator/aggregate attach) — fine for `/f/[slug]`, but affinity on entity signals needs the full `getItems()` merge. The map uses `getItems()`.
...
At the widest scale, the reader sees a global, pannable honeycomb containing content from the complete Gradiente ecosystem. Content attributed to franjas and users forms recognizable local clusters through shared metadata, not through manually drawn sections or permanent category columns.

Selecting a franja or user does not replace the map with a disconnected page. The camera moves into the existing cluster. Coarse cells resolve into detailed content cells while preserving the underlying honeycomb coordinates. At the focused scale, a contextual identity strip appears and the cluster reorganizes locally according to content affinity and current HL.

The central rules are:

1. **The honeycomb is navigation.** No conventional franja-page submenu, category tabs, or secondary card grid.
2. **Every terrain unit carries content.** No decorative, blank, category-only, statistic-only, franja-name, or generic text hexes.
...
6. **Identity is contextual chrome.** Franja/user information lives in a dedicated strip, not in content real estate.
7. **The map is global and deterministic.** It is not a personalized recommendation surface and uses no behavioral profile.

## Goals

- Make franjas and users feel native to Gradiente rather than bolted-on destination pages.
...
- Reuse the real content model and overlay readers rather than creating a parallel franja-only publishing universe.

## Non-goals

- A literal geographic map of Mexico or any other territory.
- A conventional directory of franja logos.
...
- Moving authoring or franja administration onto the public map.
...
The close zoom level for one franja or user. The selected cluster expands in place and gains contextual identity chrome.

### Identity strip

The non-terrain surface containing a franja or user's identity information. The Club Japan concept uses a Japanese vinyl obi as the model.
...
- Content from franjas, individual creators, editorial staff, marketplace sellers, venues, labels, artists, and collectives shares one lattice.
- Related content forms local neighborhoods automatically.
- Franja/user clusters can be recognized through the density and attribution of their content, but there are no permanent franja-colored landmasses.
- Franja names and logos do not consume hexes.
...
### Level 2 — franja focus

The franja cluster expands to occupy most of the viewport while remaining embedded in the global terrain.

- Only content attributed to, created by, sold by, or explicitly about the selected franja appears inside the focused perimeter.
- Neighboring global terrain remains partially visible beyond the perimeter.
- Internal content reflows by franja-local affinity and HL.
...
Franjas are present globally, but they are not rendered as colored continents.

A franja's global presence emerges from:

- published items with `franja_id = franja.id`;
- franja-authored marketplace listings;
- items explicitly related to the franja through structured entity/relationship data;
- items published by franja team members when explicit franja attribution is enabled;
- the franja identity record used only for selection, metadata, SEO, and the focus strip.

The franja record itself is not a content hex.

At global scale, a franja with many mutually related items will naturally produce a more recognizable cluster. A franja with sparse content may occupy a small neighborhood until its archive grows.

This is a consequence of the content graph, not a manually assigned territory size.

## Franja-focused content eligibility
...
1. **Direct attribution:** `items.franja_id` equals the selected franja.
2. **Marketplace ownership:** the listing belongs to the selected franja.
3. **Structured relationship:** the item links to the franja through a future `item_entities` or equivalent relation.
4. **Franja-team publishing:** a team member explicitly selected “vincular con mi franja” during authoring.
5. **Editorial association:** an admin/editor explicitly attached the franja entity.
...
The Club Japan concept uses a Japanese vinyl obi, but the structural requirement is broader: every franja gets a compact, distinctive identity strip without consuming honeycomb cells.
...
- fixed within the franja focus viewport;
...
- should be franja-customizable within a controlled Gradiente template.
...
- sparse identity clusters may borrow contextual neighboring content at global/approach scales, but franja focus eligibility remains strict.
...
- Selecting a franja/user cluster animates to its focus scale.
...
- `/f/[slug]` resolves directly to the franja focus state;
...
- pan and zoom;
- focus a franja or user;
- open content;
- follow franja/user contact links;
...
- use [[MiFranjaSection]] for franja profile, team, publishing, marketplace, and listing management;
...
- whether creator clusters can overlap franja clusters when the creator publishes on behalf of a franja.

## Marketplace behavior

Marketplace listings are content nodes, not a separate marketplace district.

- A listing is eligible inside a franja focus cluster when owned by that franja.
...
- [[Guides Not Gatekeepers]] — editorial content continues to coexist with community and franja content.
- [[Franja Authoring]] — franja-created material remains explicitly attributed.
- [[Overlay System]] — spatial cards still open the canonical detail experiences.

### Relaxed or superseded if adopted

- [[Franjas Isolation]] — franjas would no longer live only in a separate rail/route system, although franja identity records still must not masquerade as editorial content cards.
- [[Contained Single Surface]] — the canvas becomes the shared surface, while canonical focus routes remain addressable.
- [[Franja Page]] — the current dossier becomes the server/share entry point for a spatial franja focus state.
...
- users and franja team membership.

Likely additions:

- structured franja/entity relations for “explicitly about” content;
...
- Screen readers need identity context such as “Club Japan, franja focus, 12 published items.” This is semantic copy, not a visible vanity metric.
...
- the current [[FranjasDrawer]] becomes obsolete only after the canvas provides equivalent discovery;
...
### Phase 3 — franja focus

- Map `/f/[slug]` to a franja focus state.
- Add the franja identity-strip template.
- Enforce explicit focus eligibility.
- Integrate marketplace nodes.
- Preserve franja dashboard management.
...
- Remove or archive obsolete franja rail/dossier surfaces only after analytics-free usability testing and team sign-off.
...
- [ ] No franja name, logo, category, statistic, or decorative filler consumes a content hex.
- [ ] Franja/user clusters emerge from attribution and affinity rather than manual regions.
...
- [ ] Selecting a franja/user zooms into the existing cluster rather than replacing the surface.
...
- [ ] Direct `/f/[slug]` and `/u/[username]` visits render the correct focus state.

### Focused franja

- [ ] The identity strip contains franja identity/contact data and no category navigation.
- [ ] Every internal card is explicitly eligible for the selected franja.
...
1. What makes a franja cluster discoverable at global scale without putting a permanent franja-name hex in the terrain?
...
4. Does a focused franja perimeter have a visible neutral outline, or is ownership communicated only by the identity strip and eligibility?
5. Should the global map include unattributed house-editorial content between franja/user neighborhoods?
...
10. Should franja-focused content include items merely about the franja, or only explicitly attributed/published items?
...
15. Which existing franja surfaces remain as fallback during migration?
