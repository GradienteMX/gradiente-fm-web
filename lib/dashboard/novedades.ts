...
//
// Follows are EXPLICIT `{kind: 'franja' | 'genre', key}` choices stored in
...
export interface FranjaOption {
  id: string // ContentItem id of the franja row — the follow key for kind 'franja'
  slug: string
  title: string
  imageUrl?: string
}
...
// 60s tick). Franjas never enter content feeds — their profile rows are the
...
// The picker's real franja catalogue (§3.5 — the empty state IS the picker).
// Franja rows are `items` type='franja'; the row id doubles as the
// `franja_id` content items carry, so it IS the follow key.
export async function fetchFranjaOptions(): Promise<FranjaOption[]> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('items')
  ...
  if (error) {
    console.error('[fetchFranjaOptions]', error)
    return []
  }
...
// query). A franja follow matches attribution (`item.franjaId`); a genre
...
export function filterByFollows(
  ...
  const franjaKeys = new Set<string>()
  const genreKeys = new Set<string>()
  for (const f of follows) {
    if (f.kind === 'franja') franjaKeys.add(f.key)
    else genreKeys.add(f.key)
  }
  return items.filter(
    (item) =>
      (item.franjaId !== undefined && franjaKeys.has(item.franjaId)) ||
